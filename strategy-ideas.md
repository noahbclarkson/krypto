...
### 27. 4h Multi-Timeframe Turtle — Genuinely Untested

All walk-forward validation is on daily bars (310 trades / 8 years). 4h bars would give ~6x more trades and potentially catch shorter-cycle breakouts.

**Test:** Adapt `turtle_chandelier_walkforward.rs` for 4h OHLCV. Fresh EP sweep (don't port EP=21). 9-universe, 6 windows.

**Hypothesis:** 2026 YTD is bad partly because daily Turtle misses short-cycle breakouts. 4h entries catch mini-trends that daily misses.

**Risk:** More trades × fees. Need explicit fee sweep: 10bp, 15bp, 20bp RT per side.

**Status:** Genuinely new territory. Different from failed 4h mean reversion (#1) — this is the strategy class that works.
