//! Symbol-mapping audit for the rolling pooled linear alpha model.
//!
//! Goal:
//! - test whether the richer-model edge depends on the score being assigned to the *right symbol*
//!   or whether a lot of the result survives generic cross-sectional ranking / book construction
//! - keep the same fair daily harness and compare a sharper control than shuffled labels alone
//!
//! Compared rows:
//! - LinearAlpha(panel): original rolling pooled ridge model
//! - LinearAlpha(shuffled_y): training labels are rotated inside each rolling window
//! - LinearAlpha(score_rotated): fit the real model, but rotate predicted scores across symbols each day
//!   before turning them into trades, preserving score distribution while breaking symbol mapping
...
async fn main() -> Result<()> {
    println!("=== CROSS-SECTIONAL LINEAR MODEL MAPPING AUDIT ===\n");
    ...
    println!(
        "Controls: shuffled_y rotate={} | score_rotated rotate={}\n",
        SHUFFLE_ROTATE_BY, SCORE_ROTATE_BY
    );
    ...
    write_snapshots(&summaries, &timestamp)?;

    println!("\nInterpretation:");
    println!("- shuffled_y tests feature->future label dependence inside training.");
    println!("- score_rotated tests whether the score must stay attached to the right symbol to keep working.");
    println!("- if score_rotated remains too strong, the remaining edge is likely generic ranking/book structure, not symbol-specific alpha.");
...
fn generate_all_signals(
    ...
    out.insert(
        ...
        generate_linear_alpha_signals(data_cache, feature_set, symbols, false, false)?,
    );
    out.insert(
        StrategyKind::LinearAlphaShuffled,
        generate_linear_alpha_signals(data_cache, feature_set, symbols, true, false)?,
    );
    out.insert(
        StrategyKind::LinearAlphaScoreRotated,
        generate_linear_alpha_signals(data_cache, feature_set, symbols, false, true)?,
    );
...
fn generate_linear_alpha_signals(
    ...
    for i in WARMUP_BARS.max(TRAIN_BARS)..(n.saturating_sub(HOLD_BARS + 1)) {
        ...
        if rotate_scores && scores.len() > 1 {
            let score_values: Vec<f64> = scores.iter().map(|(_, s)| *s).collect();
            let len = score_values.len();
            for (idx, (_, score)) in scores.iter_mut().enumerate() {
                *score = score_values[(idx + SCORE_ROTATE_BY) % len];
            }
        }
...
fn write_snapshots(summaries: &[UniverseSummary], timestamp: &str) -> Result<()> {
    let snapshots_dir = PathBuf::from("snapshots");
    fs::create_dir_all(&snapshots_dir)?;

    let mut md = String::new();
    md.push_str("# Cross-Sectional Linear Model Mapping Audit\n\n");
    md.push_str("Compares the real rolling panel model against shuffled-label and score-rotation controls plus MACD+Regime.\n\n");
    for summary in summaries {
        md.push_str(&format!("## {}\n\n", summary.label));
    ...
    fs::write(
        snapshots_dir.join("cross_sectional_linear_model_mapping_latest.md"),
        &md,
    )?;
    fs::write(
        snapshots_dir.join("cross_sectional_linear_model_mapping_latest.csv"),
        &csv,
    )?;
    fs::write(
        snapshots_dir.join(format!(
            "cross_sectional_linear_model_mapping_{}.md",
            timestamp
        )),
        &md,
    )?;
    fs::write(
        snapshots_dir.join(format!(
            "cross_sectional_linear_model_mapping_{}.csv",
            timestamp
        )),
        &csv,
    )?;
