...
async fn main() -> Result<()> {
    ...
    println!("{}", "  STRATEGY PARAMETER SWEEP (ATR-BASED STOPS)".bright_cyan().bold());
    println!("{}", "━".repeat(70).bright_cyan());
    println!("  ATR stop multipliers: {:?}", ATR_MULTS);
    println!("  ATR TP multipliers:   {:?}", TP_MULTS);
    println!("  Strategies: {:?}", STRATEGIES);
    println!("  Symbols:    {:?}", SYMBOLS);
    println!("  Ranked by:  Annualised Sharpe (min {} trades)", MIN_TRADES);
    println!();
        ...
        for int in INTERVALS {
            ...
            match loader.fetch_data(sym, int, CANDLES).await {
                Ok(df) => {
                    ...
                    println!("{} bars  |  median ATR = {:.2}%", df.height(), atr_pct * 100.0);
    ...
    println!("{}", "  TOP 20 COMBINATIONS (ranked by annualised Sharpe)".bright_cyan().bold());
    println!("{}", "━".repeat(110).bright_cyan());
    println!(
        "  {:<22} {:<10} {:<4} {:>7} {:>7} {:>6} {:>7} {:>6} {:>8} {:>8} {:>8}",
        "Strategy", "Symbol", "Int", "Stop%", "TP%", "Trades", "Tr/yr", "Win%",
        "Ann.Ret%", "Ann.Shp", "TotalRet%"
    );
    println!("{}", "─".repeat(110));

    for (i, r) in results.iter().take(20).enumerate() {
        let ann_ret = if r.annualised_return_pct > 0.0 {
            format!("{:>8.1}", r.annualised_return_pct).green().to_string()
        } else {
            format!("{:>8.1}", r.annualised_return_pct).red().to_string()
        };
    ...
    for strat in STRATEGIES {
        if let Some(best) = results.iter().find(|r| r.strategy == *strat) {
            ...
            println!("    Interval:        {} ({:.1}y covered)", best.interval, best.backtest_years);
            println!("    Stop:            {:.1}× ATR = {:.2}%", best.atr_stop_mult, best.effective_stop_pct);
            println!("    TP:              {:.1}× ATR = {:.2}%", best.atr_tp_mult, best.effective_tp_pct);
            println!("    Trades:          {} ({:.1}/yr)", best.trades, best.trades_per_year);
    ...
    println!("  {:>6}  {:>12}  {:>12}  {:>6}", "Mult", "Ann.Return%", "Ann.Sharpe", "Runs");
    for (mult_x10, (sum_ret, sum_shp, count)) in &mult_stats {
        ...
        println!("  {:>6.1}×   {}   {:>12.2}  {:>6}", mult, ret_str, avg_shp, count);
    }

    // ── Timeframe analysis ───────────────────────────────────────────────────
    println!("\n  Avg metrics by timeframe:");
    println!("  {:>4}  {:>12}  {:>12}  {:>8}  {:>6}", "Int", "Ann.Return%", "Ann.Sharpe", "Tr/yr", "Runs");
    let mut int_stats: HashMap<String, (f64, f64, f64, usize)> = HashMap::new();
    for r in &results {
        let e = int_stats.entry(r.interval.clone()).or_insert((0.0, 0.0, 0.0, 0));
    ...
    int_vec.sort_by(|a, b| (b.1 .1 / b.1 .3 as f64).partial_cmp(&(a.1 .1 / a.1 .3 as f64)).unwrap());
