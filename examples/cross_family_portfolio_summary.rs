//! Integrated cross-family capped-book portfolio summary.
//!
//! Purpose:
//! - stop comparing separate family winners across separate audit files
//! - force representative candidates from trend, ensemble, ticker-interaction,
//!   and exogenous breadth through the same realistic portfolio lens
//! - treat this as a cross-family trust summary, not a promotion script
//!
...
const SNAPSHOT_LATEST_MD: &str = "snapshots/cross_family_portfolio_summary_latest.md";
const SNAPSHOT_LATEST_CSV: &str = "snapshots/cross_family_portfolio_summary_latest.csv";
...
impl StrategyKind {
    ...
    fn name(&self) -> &'static str {
        match self {
            ...
            Self::CrossImpactResidualBtc => "CrossImpact residual BTC",
...
async fn main() -> Result<()> {
    println!("=== CROSS-FAMILY PORTFOLIO SUMMARY ===\n");
    println!("Goal: compare representative family leaders under one realistic capped-book portfolio lens");
    ...
    for &(label, universe_symbols) in UNIVERSES {
        ...
        let cross_inputs = build_cross_impact_inputs(&data_cache, universe_symbols)?;
        let mut rows = Vec::new();

        for &strategy in StrategyKind::all() {
            let plans = build_symbol_plans(&universe, &macro_contexts, &cross_inputs, strategy)?;
...
fn build_symbol_plans(
    ...
    universe
        ...
        .map(|(symbol, df)| {
            ...
            let cross = cross_inputs
                .get(symbol)
                .with_context(|| format!("missing cross-impact inputs for {symbol}"))?;
            build_symbol_plan(df, ctx, cross, strategy)
        })
        .collect()
}
...
fn build_symbol_plan(
    df: &DataFrame,
    ctx: &MacroContext,
    cross: &CrossImpactInputs,
    strategy: StrategyKind,
) -> Result<SymbolPlan> {
    let signals = signal_for_strategy(df, ctx, cross, strategy)?;
    let strengths = strengths_for_strategy(df, ctx, cross, strategy)?;
...
fn signal_for_strategy(
    ...
    let cross_impact_residual = generate_cross_impact_signals(cross, true);

    Ok(match strategy {
        StrategyKind::SpxFollow => spx,
        StrategyKind::VixInverse => vix,
        StrategyKind::CrossImpactResidualBtc => cross_impact_residual,
...
fn strengths_for_strategy(
    ...
    match strategy {
        ...
        StrategyKind::CrossImpactResidualBtc => Ok(generate_cross_impact_strengths(cross, true)),
...
fn build_cross_impact_inputs(
    ...
    let min_len = symbols
        .iter()
        .chain(std::iter::once(&BENCHMARK))
        .filter_map(|symbol| data_cache.get(*symbol).map(|df| df.height()))
        .min()
        .ok_or_else(|| anyhow::anyhow!("empty cross-impact universe"))?;

    let mut closes = HashMap::<String, Vec<f64>>::new();
    for &symbol in symbols.iter().chain(std::iter::once(&BENCHMARK)) {
        let df = data_cache
            .get(symbol)
            .with_context(|| format!("missing cross-impact data for {symbol}"))?;
        closes.insert(
            symbol.to_string(),
            df.column("close")?
                .f64()?
    ...
    let btc_close = closes
        .get(BENCHMARK)
        .context("missing BTC close for cross-impact")?
        .clone();
    let mut out = HashMap::new();
    for &symbol in symbols {
        let target_close = closes
            .get(symbol)
            .context("missing target close for cross-impact")?
            .clone();
        ...
        for &peer in symbols {
            if peer != symbol {
                peer_closes.insert(
                    ...
                    closes
                        .get(peer)
                        .context("missing peer close for cross-impact")?
                        .clone(),
                );
            }
        }
...
fn generate_cross_impact_signals(inputs: &CrossImpactInputs, residualize_btc: bool) -> Vec<i32> {
    let strengths = generate_cross_impact_strengths(inputs, residualize_btc);
    strengths
        .into_iter()
        .map(|v| {
            if v > 0.0 {
...
fn generate_cross_impact_strengths(inputs: &CrossImpactInputs, residualize_btc: bool) -> Vec<f64> {
    let n = inputs.target_close.len();
    let mut out = vec![0.0; n];

    for i in PEER_LOOKBACK.max(BTC_BETA_LOOKBACK)..n {
        let self_ret = if residualize_btc {
            residualized_return(
                &inputs.target_close,
                &inputs.btc_close,
                i,
            ...
            pct_return(&inputs.target_close, i, PEER_LOOKBACK)
        }
        .unwrap_or(0.0);

        let self_cap = if residualize_btc { 0.012 } else { 0.015 };
        ...
        for closes in inputs.peer_closes.values() {
            let ret = if residualize_btc {
                ...
                pct_return(closes, i, PEER_LOOKBACK)
            };
            if let Some(ret) = ret {
                peer_sum += ret;
                peer_count += 1;
        ...
        let threshold = if residualize_btc { 0.020 } else { 0.040 };
        if peer_avg >= threshold && same_sign >= 2 {
            out[i] = peer_avg;
        } else if peer_avg <= -threshold && same_sign <= -2 {
            out[i] = peer_avg;
        }
    }
...
fn write_snapshot(summaries: &[UniverseSummary]) -> Result<(String, String)> {
    ...
    let archive_md = format!(
        "{}/cross_family_portfolio_summary_{}.md",
        SNAPSHOT_DIR, timestamp
    );
    let archive_csv = format!(
        "{}/cross_family_portfolio_summary_{}.csv",
        SNAPSHOT_DIR, timestamp
    );
...
fn render_markdown(summaries: &[UniverseSummary]) -> String {
    ...
    out.push_str("# Cross-Family Portfolio Summary\n\n");
