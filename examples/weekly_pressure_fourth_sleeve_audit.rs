//! Pre-declared fourth-sleeve audit: can weekly pressure earn allocator attention on top of
//! the frozen three-sleeve reference book?
//!
//! Purpose:
//! - avoid another standalone leaderboard loop after the weekly pressure benchmark survived as a
//!   secondary family
//! - force weekly pressure into the same realistic sleeve construction lens as the current
//!   `DDBudget(A/D,MACD,Small)` reference book
//! - answer a trust question, not a promotion question: does pressure diversify the book, or just
//!   repackage existing exposure?
//!
...
//!
//! Honesty note:
//! - weekly pressure here is still an OHLCV-derived signed dollar-flow proxy, not true taker-flow
...
const SNAPSHOT_LATEST_MD: &str = "snapshots/weekly_pressure_fourth_sleeve_audit_latest.md";
const SNAPSHOT_LATEST_CSV: &str = "snapshots/weekly_pressure_fourth_sleeve_audit_latest.csv";
...
enum PortfolioKind {
    ...
    DdBudgetAllThree,
    DdBudgetAllFour,
    DdBudgetTrendPressureSmall,
    DdBudgetAdPressureSmall,
}
...
impl PortfolioKind {
    fn all() -> &'static [PortfolioKind] {
        &[
            ...
            Self::DdBudgetTrendPressureSmall,
            Self::DdBudgetAdPressureSmall,
        ]
    }

    fn name(&self) -> &'static str {
        match self {
            Self::PressureOnly => "WeeklyPressure+Small only",
            Self::DdBudgetAllThree => "DDBudget(A/D,MACD,Small)",
            Self::DdBudgetAllFour => "DDBudget(A/D,MACD,Small,Pressure)",
            Self::DdBudgetTrendPressureSmall => "DDBudget(MACD,Small,Pressure)",
            Self::DdBudgetAdPressureSmall => "DDBudget(A/D,Small,Pressure)",
        }
    }
}
...
async fn main() -> Result<()> {
    println!("=== WEEKLY PRESSURE FOURTH-SLEEVE AUDIT ===\n");
    println!("Question: does WeeklyPressure+Small add enough diversification to earn allocator attention on top of the frozen three-sleeve book?");
    println!("Allocators: pressure standalone vs a few pre-declared DDHard sleeve books.");
    println!(
    ...
    println!("Honesty note: weekly pressure is still an OHLCV-derived flow proxy, not true taker-flow.\n");
...
fn included_strategies(portfolio: PortfolioKind) -> &'static [StrategyKind] {
    match portfolio {
        PortfolioKind::PressureOnly => &[StrategyKind::WeeklyPressureSmall],
        ...
        PortfolioKind::DdBudgetTrendPressureSmall => &[
            StrategyKind::MacdRegime,
            StrategyKind::SmallByDollarVol,
            StrategyKind::WeeklyPressureSmall,
        ],
        PortfolioKind::DdBudgetAdPressureSmall => &[
            StrategyKind::AdMomentum,
            StrategyKind::SmallByDollarVol,
            StrategyKind::WeeklyPressureSmall,
        ],
...
fn sleeve_weight(
    ...
    match portfolio {
        PortfolioKind::PressureOnly if strategy == StrategyKind::WeeklyPressureSmall => {
        ...
        PortfolioKind::DdBudgetAllFour => match strategy {
            ...
            | StrategyKind::MacdRegime
            | StrategyKind::SmallByDollarVol
            | StrategyKind::WeeklyPressureSmall => 0.25 * ddhard_exposure(dd_pct, recovery_ratio),
        },
        PortfolioKind::DdBudgetTrendPressureSmall => match strategy {
            StrategyKind::MacdRegime
            | StrategyKind::SmallByDollarVol
            | StrategyKind::WeeklyPressureSmall => {
                (1.0 / 3.0) * ddhard_exposure(dd_pct, recovery_ratio)
        ...
        PortfolioKind::DdBudgetAdPressureSmall => match strategy {
            StrategyKind::AdMomentum
            | StrategyKind::SmallByDollarVol
            | StrategyKind::WeeklyPressureSmall => {
                (1.0 / 3.0) * ddhard_exposure(dd_pct, recovery_ratio)
...
fn write_snapshot(universe_summaries: &[UniverseSummary]) -> Result<(String, String)> {
    ...
    let archive_md = format!(
        "{}/weekly_pressure_fourth_sleeve_audit_{}.md",
        SNAPSHOT_DIR, ts
    );
    let archive_csv = format!(
        "{}/weekly_pressure_fourth_sleeve_audit_{}.csv",
        SNAPSHOT_DIR, ts
    );

    let mut md = String::from("# Weekly Pressure Fourth-Sleeve Audit\n\n");
