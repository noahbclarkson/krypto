//! BTC→XRP Lead-Lag Pair Trade — DAILY RESOLUTION — Walk-Forward Validation
//!
//! Track C broadening: Does the BTC→ETH lead-lag effect generalize to XRP?
//!
//! Signal: BTC daily return > threshold → long XRP next day
...
async fn main() -> Result<()> {
    println!("{}", "═".repeat(72));
    println!("  BTC→XRP LEAD-LAG @ 1d — WALK-FORWARD VALIDATION");
    println!("{}", "═".repeat(72));

    let loader = DataLoader::new(None, None);

    let btc_df = loader.fetch_data("BTCUSDT", "1d", CANDLES).await?;
    let xrp_df = loader.fetch_data("XRPUSDT", "1d", CANDLES).await?;
    ...
    let mut matched = 0;
    for i in 0..n {
        if let Some(&c) = btc_map.get(&xrp_times[i]) {
            btc_aligned[i] = c;
            matched += 1;
        }
    }
    ...
    println!("XRP: {} bars | {} → {}", xrp_closes.len(), epoch_ms_to_date(xrp_times[0]), epoch_ms_to_date(xrp_times[xrp_times.len()-1]));
    ...
    // B&H baseline
    let bnh_ret = (xrp_closes[n-1] / xrp_closes[TRAIN_BARS] - 1.0) * 100.0;
    println!("XRP B&H (from W0 start): {:.1}%", bnh_ret);

    // ── Sweep ──
    println!("\n{}", "═".repeat(72));
    println!("  LONG-ONLY RESULTS (BTC surge → long XRP)");
    println!("{}", "═".repeat(72));
    ...
    for &lead in LEAD_LENGTHS {
        for &thresh in THRESHOLDS {
            ...
            for wi in 0..total_windows {
                ...
                let r = run_lead_lag(&btc_aligned, &xrp_closes, lead, thresh, ts, te);
    ...
    println!("  XRP B&H: {:.1}%", bnh_ret);

    // ── Verdict ──
    println!("\n{}", "═".repeat(72));
    if best_sharpe > 1.5 && best_pass as f64 / best_results.len() as f64 > 0.5 {
        println!("  ✅ BTC→XRP lead-lag at 1d: Sharpe {:.2}, {}/{} pass", best_sharpe, best_pass, best_results.len());
        println!("     Lead={} bars, threshold={:.1}%, hold={} bars", best_config.0, best_config.1, HOLD_BARS);
    } else if best_sharpe > 0.5 {
        println!("  ⚠️  BTC→XRP lead-lag at 1d: Marginal Sharpe {:.2}, {}/{} pass", best_sharpe, best_pass, best_results.len());
    } else {
        println!("  ❌ BTC→XRP lead-lag at 1d: Negative/marginal Sharpe {:.2}", best_sharpe);
    }
    println!("{}", "═".repeat(72));
