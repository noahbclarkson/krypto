...
async fn main() -> anyhow::Result<()> {
    ...
    if !bars.is_empty() {
        ...
        println!("  Time range: {} to {}", first.time.format("%Y-%m-%d %H:%M"), last.time.format("%Y-%m-%d %H:%M"));
        println!("  Price range: ${:.2} - ${:.2}", bars.iter().map(|b| b.low).fold(f64::MAX, f64::min), bars.iter().map(|b| b.high).fold(f64::MIN, f64::max));
    }
    println!();
    ...
    let strategies: Vec<(&str, Box<dyn Strategy>)> = vec![
        ...
        ("RSI Mean Reversion", Box::new(RsiStrategy::new(14, 30.0, 70.0))),
    ];

    println!("{}", format!("Running {} strategies...", strategies.len()).cyan());
    println!("{}", "=".repeat(100));
    println!();
    ...
    println!("{} Best Return: {} ({:.2}%)", "🏆".yellow(), best.strategy.bold(), best.total_return_pct);

    let best_accuracy = summaries.iter()
        .max_by(|a, b| a.directional_accuracy.partial_cmp(&b.directional_accuracy).unwrap())
        .unwrap();

    println!("{} Best Accuracy: {} ({:.2}%)", "🎯".yellow(), best_accuracy.strategy.bold(), best_accuracy.directional_accuracy);

    let lowest_dd = summaries.iter()
        .min_by(|a, b| a.max_drawdown_pct.partial_cmp(&b.max_drawdown_pct).unwrap())
        .unwrap();

    println!("{} Lowest Drawdown: {} ({:.2}%)", "🛡️".yellow(), lowest_dd.strategy.bold(), lowest_dd.max_drawdown_pct);
    println!();
    ...
    if passing.is_empty() {
        ...
        println!("✅ {} strateg(y/ies) meet the {:.0}% accuracy target:", passing.len(), target);
        for s in passing {
