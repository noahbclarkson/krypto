//! BTC→ETH Lead-Lag Pair Trade — DAILY RESOLUTION — Walk-Forward Validation
//!
//! Extended version of the 4h test. Daily bars give:
//! - ~3000 bars (~8 years) instead of 8000 4h bars (~3 years)
//! - More OOS windows for better statistical power
//! - Daily close is a more meaningful price signal
//!
//! Signal: BTC daily return > threshold → long ETH next day
...
fn run_lead_lag(
    ...
    while i < test_end.saturating_sub(lead_len + HOLD_BARS + 1) {
        ...
        if btc_ret > threshold {
            ...
            if entry_idx >= eth.len() || exit_idx <= entry_idx {
                i += 1;
                continue;
            }
...
async fn main() -> Result<()> {
    println!("{}", "═".repeat(72));
    println!("  BTC→ETH LEAD-LAG @ 1d — WALK-FORWARD VALIDATION");
    println!("{}", "═".repeat(72));
    ...
    // B&H baseline
    let bnh_ret = (eth_closes[n-1] / eth_closes[TRAIN_BARS] - 1.0) * 100.0;
    println!("ETH B&H (from W0 start): {:.1}%", bnh_ret);

    // ── Sweep ──
    println!("\n{}", "═".repeat(72));
    println!("  LONG-ONLY RESULTS (BTC surge → long ETH)");
    println!("{}", "═".repeat(72));
    ...
    for &lead in LEAD_LENGTHS {
        for &thresh in THRESHOLDS {
            ...
            for wi in 0..total_windows {
                ...
                let r = run_lead_lag(&btc_aligned, &eth_closes, lead, thresh, ts, te);
    ...
    while i < n.saturating_sub(HOLD_BARS + 1) {
        let entry = eth_closes[i + 1];
        let exit_idx = (i + 1 + HOLD_BARS).min(n - 1);
        let exit = eth_closes[exit_idx];
    ...
    if best_sharpe > 1.5 && best_pass as f64 / best_results.len() as f64 > 0.5 {
        println!("  ✅ BTC→ETH lead-lag at 1d: Sharpe {:.2}, {}/{} pass", best_sharpe, best_pass, best_results.len());
        println!("     Lead={} bars, threshold={:.1}%, hold={} bars", best_config.0, best_config.1, HOLD_BARS);
    } else if best_sharpe > 0.5 {
        println!("  ⚠️  BTC→ETH lead-lag at 1d: Marginal Sharpe {:.2}, {}/{} pass", best_sharpe, best_pass, best_results.len());
    } else {
        println!("  ❌ BTC→ETH lead-lag at 1d: Negative/marginal Sharpe {:.2}", best_sharpe);
    }
    println!("{}", "═".repeat(72));
