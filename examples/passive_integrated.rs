...
async fn main() -> Result<()> {
    ...
    println!(
        "{}",
        "  INTEGRATED PASSIVE EXECUTION — FDUSD 0% Maker Fees"
            .bright_cyan()
            .bold()
    );
    ...
    results.sort_by(|a, b| {
        b.maker
            .total_return_pct
            .partial_cmp(&a.maker.total_return_pct)
    ...
    for r in &results {
        let maker_str = if r.maker.total_return_pct > 0.0 {
            format!("{:>9.1}%", r.maker.total_return_pct)
                .green()
                .to_string()
        } else {
            format!("{:>9.1}%", r.maker.total_return_pct)
    ...
    let profitable = results
        .iter()
        .filter(|r| r.maker.total_return_pct > 0.0)
        .count();
