//! CHAND_PERIOD Extended Sweep: 5-100 step 1 (96 values)
//! Production params except CHAND_PERIOD: EP=21, ATR_P=24, ATR_M=2.0, CHAND_M=1.50, HM=45, CAP=3, VL=2
//! 9 universes x ~9 windows x 96 values
//! Exports: per-window results + equity curves for top candidates
...
// 96 candidate Chandelier ATR periods: 5 to 100 step 1
...
const CSV_OUT: &str = "snapshots/chand_period_extended_wf.csv";
const EQUITY_OUT: &str = "snapshots/chand_period_extended_equity.csv";
                ...
                if bar >= EP + 1 && bar < sd.close.len() {
                    if turtle_signal(&sd.close, EP, bar) {
                        ...
                        if let Some(&exit_px) = sd.close.get(exit_bar) {
                            ...
                            // Push equity for each bar of the hold
...
async fn main() -> Result<()> {
    let t0 = Instant::now();
    eprintln!("==== CHAND_PERIOD Extended Sweep: 5-100 step 1 (96 values) ====");
    ...
    for &chand_p in CHAND_PERIODS {
        ...
        for &(label, symbols) in UNIVERSES {
            ...
            for wi in 0..max_windows {
                ...
                // Sample equity curve: step through the hold
...
=== Top 10 by Sharpe ===");
    ...
    for (&cp, &(sum_s, sum_trades, sum_pass, _cnt)) in sorted.iter().take(10) {
