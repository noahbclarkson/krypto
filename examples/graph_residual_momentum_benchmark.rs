//! Graph-weighted residual momentum breadth benchmark.
//!
//! Goal:
//! - test a more structural Track C family after the simple peer-cluster,
//!   latent-cluster, and cluster-factor residual rules all failed
//! - ask whether symbols keep trending after outperforming or underperforming a
//!   weighted graph-neighbour factor rather than a raw peer mean or coarse cluster
//! - keep the same fair daily harness used elsewhere so results stay comparable
//!
//! Construction:
//! - use an early fixed window to estimate pairwise return correlations across the universe
//! - for each symbol, keep its top positive-correlation neighbours and weight them by
//!   calibration-period correlation strength
//! - estimate a rolling BTC beta and a rolling graph-factor beta for each symbol
//! - compute a 21-bar residual momentum score after removing both the BTC move and
//!   the weighted neighbour factor exposure
...
impl StrategyKind {
    fn name(&self) -> &'static str {
        match self {
            Self::GraphResidualMomentum => "GraphResidualMomentum",
...
async fn main() -> Result<()> {
    println!("=== GRAPH-WEIGHTED RESIDUAL MOMENTUM BREADTH BENCHMARK ===\n");
    ...
    println!("Graph top peers: {}", GRAPH_TOP_PEERS);
    ...
    println!("\nInterpretation:");
    println!("- GraphResidualMomentum asks whether residual momentum survives after removing both BTC market mode and a weighted graph-neighbour factor.");
    println!("- If it fails, that is still useful: it means simple daily relational breadth is not rescued just by moving from cluster means to weighted neighbour graphs.");
    println!("- If it survives selectively, that points toward even richer graph / latent-factor models rather than raw peer means.");
...
fn generate_all_signals(
    ...
    out.insert(
        StrategyKind::GraphResidualMomentum,
        generate_graph_residual_signals(data_cache, symbols)?,
    );
...
fn generate_graph_residual_signals(
    data_cache: &HashMap<String, DataFrame>,
    symbols: &[&str],
    ...
    let peer_map = assign_graph_peers(symbols, &ret_map);
    let mut graph_factor_map = HashMap::<String, Vec<f64>>::new();
    for &symbol in symbols {
        ...
        for i in 0..n {
            series[i] =
                graph_factor_return(&close_map, &peers, i, RESIDUAL_LOOKBACK).unwrap_or(f64::NAN);
        }
        graph_factor_map.insert(symbol.to_string(), series);
    }
        ...
        WARMUP_BARS.max(CALIBRATION_BARS).max(RESIDUAL_LOOKBACK)..(n.saturating_sub(HOLD_BARS + 1))
    {
        ...
        for &symbol in symbols {
            ...
            let factor_series = graph_factor_map.get(symbol).unwrap();
            let residual = graph_residual_return(
                close,
                &btc_close,
                factor_series,
                i,
...
fn graph_factor_return(
    ...
    for (peer, weight) in peers {
        if let Some(peer_close) = close_map.get(peer) {
            let peer_ret = lookback_return(peer_close, i, lookback);
            if peer_ret.is_finite() {
                weighted_sum += weight * peer_ret;
                weight_sum += weight;
            }
        }
    }
...
fn assign_graph_peers(
    ...
    for &symbol in symbols {
        ...
        for &other in symbols {
            ...
            let c = pair_corr(&corr, symbol, other);
            if c > 0.0 && c.is_finite() {
                peers.push((other.to_string(), c));
            }
        }
        peers.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap_or(std::cmp::Ordering::Equal));
        peers.truncate(GRAPH_TOP_PEERS);
        out.insert(symbol.to_string(), peers);
    }
    out
}
...
fn graph_residual_return(
    ...
    let cluster_factor_ret = cluster_factor.get(i).copied()?;
    if !cluster_factor_ret.is_finite() {
        return None;
    }
    let (beta_btc, beta_cluster) =
        rolling_two_factor_betas(asset, btc, cluster_factor, i, lookback, beta_lookback)?;
    Some(asset_ret - beta_btc * btc_ret - beta_cluster * cluster_factor_ret)
}
...
fn write_snapshot_files(
    ...
    fs::write("snapshots/graph_residual_momentum_benchmark_latest.md", &md)?;
    fs::write(
        "snapshots/graph_residual_momentum_benchmark_latest.csv",
        &csv,
    )?;
    fs::write(
        format!("snapshots/graph_residual_momentum_benchmark_{}.md", ts),
        &md,
    )?;
    fs::write(
        format!("snapshots/graph_residual_momentum_benchmark_{}.csv", ts),
        &csv,
    )?;
    Ok(())
}
