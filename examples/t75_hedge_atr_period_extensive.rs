//! T75: HEDGE_ATR_PERIOD live-path extensive hyperopt.
//!
//! Parameter audited: the hardcoded BTC hedge ATR period (`21`) in `src/live/bot.rs`.
//! This is not exposed in LiveConfig and had no standalone justification: the live
//! path uses `BTC ATR(period) > 45th percentile of 252 daily true ranges` to size
//! entries by HEDGE_SIZE_MULT=0.40.
//!
//! Sweep: period = 5..=100, step 1 (96 integer values).
//! Validation: 9 universes × 252d walk-forward windows, fixed current live params.
//! Export:
//! - /home/ubuntu/.openclaw/workspace-krypto/krypto/snapshots/t75_hedge_atr_period_summary.csv
//! - /home/ubuntu/.openclaw/workspace-krypto/krypto/snapshots/t75_hedge_atr_period_windows.csv
//! - /home/ubuntu/.openclaw/workspace-krypto/krypto/snapshots/t75_hedge_atr_period_equity.csv
//! - /home/ubuntu/.openclaw/workspace-krypto/krypto/snapshots/t75_hedge_atr_period_summary.json
...
const BASELINE_HEDGE_ATR_PERIOD: usize = 21;
const HEDGE_PERIOD_MIN: usize = 5;
const HEDGE_PERIOD_MAX: usize = 100;
const HEDGE_PERIOD_STEP: usize = 1;
const FRESHNESS_COOLDOWN: usize = 0;

const SUMMARY_OUT: &str = "/home/ubuntu/.openclaw/workspace-krypto/krypto/snapshots/t75_hedge_atr_period_summary.csv";
const WINDOWS_OUT: &str = "/home/ubuntu/.openclaw/workspace-krypto/krypto/snapshots/t75_hedge_atr_period_windows.csv";
const EQUITY_OUT: &str = "/home/ubuntu/.openclaw/workspace-krypto/krypto/snapshots/t75_hedge_atr_period_equity.csv";
const SUMMARY_JSON_OUT: &str = "/home/ubuntu/.openclaw/workspace-krypto/krypto/snapshots/t75_hedge_atr_period_summary.json";
...
fn hedge_active(btc: &SymData, idx: usize, hedge_period: usize) -> bool {
    let n = idx + 1;
    if hedge_period == 0 || n < 252 + hedge_period {
        return false;
    }
    ...
    for idx in test_start..test_end {
        for sym in symbols {
            ...
            if let Some(mut pos) = positions.remove(sym) {
                ...
                if hedge_active(btc, idx, hedge_period) {
...
async fn main() -> Result<()> {
    println!("=== T75: HEDGE_ATR_PERIOD LIVE-PATH EXTENSIVE SWEEP ===");
    println!("Sweep hedge_period={}..={} step {}", HEDGE_PERIOD_MIN, HEDGE_PERIOD_MAX, HEDGE_PERIOD_STEP);
    println!("Params: EP={}, ATR({}, {:.2}), HM={}, CAP={}, ATR_RANK(AP={}, LB={}, T={:.1}), HEDGE_PCT={:.2}, HEDGE_SIZE={:.2}, baseline hedge_period={}", TURTLE_EP, TURTLE_ATR_PERIOD, TURTLE_ATR_MULT, HOLD_MAX, POSITION_CAP, REGIME_ATR_PERIOD, REGIME_LOOKBACK, ATR_RANK_THRESHOLD, HEDGE_ATR_PCT, HEDGE_SIZE_MULT, BASELINE_HEDGE_ATR_PERIOD);
    ...
    writeln!(window_csv, "hedge_atr_period,universe,window,final_equity,return_pct,sharpe,max_dd_pct,trades,win_rate_pct,pass")?;
    ...
    for hedge_period in (HEDGE_PERIOD_MIN..=HEDGE_PERIOD_MAX).step_by(HEDGE_PERIOD_STEP) {
        for (u_name, u_syms) in UNIVERSES {
            ...
            for w in 0..windows {
                ...
                let res = run_sim(&data, &symbols, test_start, test_end, hedge_period);
                ...
                let a = agg.entry(hedge_period).or_default();
    ...
    writeln!(summary_csv, "hedge_atr_period,pass_count,total_windows,pass_rate_pct,avg_sharpe,avg_return_pct,avg_max_dd_pct,total_trades,win_rate_pct")?;
    let mut rows = Vec::new();
    for hedge_period in (HEDGE_PERIOD_MIN..=HEDGE_PERIOD_MAX).step_by(HEDGE_PERIOD_STEP) {
        let a = agg.get(&hedge_period).cloned().unwrap_or_default();
        ...
        writeln!(
            summary_csv,
            "{},{},{},{:.4},{:.6},{:.4},{:.4},{},{:.4}",
            hedge_period, a.pass, a.total, pass_rate, avg_sharpe, avg_ret, avg_dd, a.trades, win_rate
        )?;
        rows.push((
            hedge_period, a.pass, a.total, pass_rate, avg_sharpe, avg_ret, avg_dd, a.trades, win_rate,
        ));
    ...
    println!("\nTop 10 hedge periods by robustness (pass rate, then Sharpe):");
    for (period, pass, total, pr, sh, ret, dd, trades, wr) in rows.iter().take(10) {
        println!("P={:3} | pass {:2}/{:<2} ({:5.1}%) | Sharpe {:6.3} | Ret {:7.2}% | DD {:5.2}% | trades {:4} | WR {:5.1}%", period, pass, total, pr, sh, ret, dd, trades, wr);
    }

    let mut json = File::create(SUMMARY_JSON_OUT)?;
    writeln!(json, "{{")?;
    writeln!(json, "  \"parameter\": \"HEDGE_ATR_PERIOD\",")?;
    writeln!(json, "  \"baseline\": {},", BASELINE_HEDGE_ATR_PERIOD)?;
    if let Some((winner_period, _, _, _, _, _, _, _, _)) = rows.first() {
        writeln!(json, "  \"winner\": {},", winner_period)?;
    }
    writeln!(
        json,
        "  \"selection_rule\": \"pass_rate_desc_then_avg_sharpe_desc_then_avg_max_dd_asc\","
    )?;
    writeln!(
        json,
        "  \"values_tested\": {}",
        ((HEDGE_PERIOD_MAX - HEDGE_PERIOD_MIN) / HEDGE_PERIOD_STEP) + 1
    )?;
    writeln!(json, "}}")?;

    // Export Base5 full-history equity curves for every period. The chart script
    ...
    let base5_min_len = base5_symbols
        .iter()
        .filter_map(|s| data.get(s).map(|d| d.close.len()))
        .min()
        .unwrap_or(0);
    let mut eq_csv = File::create(EQUITY_OUT)?;
    writeln!(eq_csv, "hedge_atr_period,step,equity")?;
    for hedge_period in (HEDGE_PERIOD_MIN..=HEDGE_PERIOD_MAX).step_by(HEDGE_PERIOD_STEP) {
        let res = run_sim(&data, &base5_symbols, WARMUP_BARS, base5_min_len, hedge_period);
        for (step, eq) in res.equity_curve.iter().enumerate() {
            writeln!(eq_csv, "{},{},{:.8}", hedge_period, step, eq)?;
        }
    }
