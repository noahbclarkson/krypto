...
async fn main() -> anyhow::Result<()> {
    ...
    let sma_params: Vec<(usize, usize)> =
        vec![(5, 10), (5, 20), (10, 20), (10, 30), (20, 50), (20, 100)];

    let ema_params: Vec<(usize, usize)> = vec![(5, 10), (5, 20), (10, 20), (10, 30), (20, 50)];
    ...
    for (fast, slow) in &sma_params {
        ...
        if summary.directional_accuracy >= min_directional_accuracy
    ...
    for summary in &all_summaries {
        let passed = summary.directional_accuracy >= min_directional_accuracy
            && summary.total_trades >= min_trades;
    ...
    if !passed_summaries.is_empty() {
        println!("\n{}", "=".repeat(90));
        println!(
            "STRATEGIES PASSING >52% DIRECTIONAL ACCURACY (min {} trades)",
            min_trades
        );
        println!("{}", "=".repeat(90));

        passed_summaries.sort_by(|a, b| {
            b.directional_accuracy
                .partial_cmp(&a.directional_accuracy)
                .unwrap()
        });
