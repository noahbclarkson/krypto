//! Separate-sleeve audit for cross-sectional FactorMomentum against the frozen three-sleeve baseline.
//!
//! Purpose:
//! - follow up the recent overlay-heavy sessions with a genuinely different Track C object
//! - test whether cross-sectional factor momentum adds anything as its own sleeve under the
...
//!
//! Honesty note:
//! - factor momentum here is still a simple daily multi-horizon price-strength object
...
const SNAPSHOT_LATEST_MD: &str = "snapshots/factor_momentum_sleeve_audit_latest.md";
const SNAPSHOT_LATEST_CSV: &str = "snapshots/factor_momentum_sleeve_audit_latest.csv";
...
enum PortfolioKind {
    ...
    MomentumOnly,
    DdBudgetAllFour,
    DdBudgetAdMacdMomentum,
    DdBudgetAdSmallMomentum,
    DdBudgetMacdSmallMomentum,
}
...
impl PortfolioKind {
    fn all() -> &'static [PortfolioKind] {
        &[
            ...
            Self::DdBudgetAdMacdMomentum,
            Self::DdBudgetAdSmallMomentum,
            Self::DdBudgetMacdSmallMomentum,
        ]
    }
    ...
    fn name(&self) -> &'static str {
        match self {
            ...
            Self::MomentumOnly => "FactorMomentum only",
            Self::DdBudgetAllFour => "DDBudget(A/D,MACD,Small,Mom)",
            Self::DdBudgetAdMacdMomentum => "DDBudget(A/D,MACD,Mom)",
            Self::DdBudgetAdSmallMomentum => "DDBudget(A/D,Small,Mom)",
            Self::DdBudgetMacdSmallMomentum => "DDBudget(MACD,Small,Mom)",
        }
    }
}
...
async fn main() -> Result<()> {
    println!("=== FACTOR MOMENTUM SLEEVE AUDIT ===\n");
    println!("Question: does FactorMomentum add anything as a separate sleeve vs the frozen A/D + MACD + Small baseline?");
    ...
    println!(
        "Honesty note: FactorMomentum = simple multi-horizon cross-sectional price strength, not a promoted alpha family.\n"
    );
    ...
    for &(label, universe_symbols) in UNIVERSES {
        ...
        let momentum_signal_map = generate_momentum_factor_signal_strengths(&universe)?;
...
fn generate_momentum_factor_signal_strengths(
    universe: &UniverseData,
) -> Result<HashMap<String, (Vec<i32>, Vec<f64>)>> {
    let n = universe
        .data
    ...
    for (symbol, df) in &universe.data {
        ...
        let vol_21 = rolling_realized_vol(close, 21);
        let mut scores = vec![0.0; n];
        for i in 126..n {
            let vol = vol_21[i].max(1e-6);
            scores[i] = 0.45 * ret_21[i] / vol + 0.35 * ret_63[i] / vol + 0.20 * ret_126[i] / vol;
        }
        raw_scores.insert(symbol.clone(), scores);
    }
...
fn build_symbol_plans(
    ...
    momentum_signal_map: &HashMap<String, (Vec<i32>, Vec<f64>)>,
) -> Result<Vec<SymbolPlan>> {
    universe
        .data
        .iter()
        .map(|(symbol, df)| {
            build_symbol_plan(df, symbol, strategy, small_signal_map, momentum_signal_map)
        })
        .collect()
}
...
fn build_symbol_plan(
    ...
    let (signals, strengths) = match strategy {
        ...
        StrategyKind::Momentum => momentum_signal_map
            .get(symbol)
            .cloned()
            .context("missing momentum sleeve signal map")?,
    };
...
fn included_strategies(portfolio: PortfolioKind) -> &'static [StrategyKind] {
    match portfolio {
        ...
        PortfolioKind::MomentumOnly => &[StrategyKind::Momentum],
        ...
        PortfolioKind::DdBudgetAdMacdMomentum => &[
            StrategyKind::AdMomentum,
            StrategyKind::MacdRegime,
            StrategyKind::Momentum,
        ],
        PortfolioKind::DdBudgetAdSmallMomentum => &[
            StrategyKind::AdMomentum,
            StrategyKind::SmallByDollarVol,
            StrategyKind::Momentum,
        ],
        PortfolioKind::DdBudgetMacdSmallMomentum => &[
            StrategyKind::MacdRegime,
            StrategyKind::SmallByDollarVol,
            StrategyKind::Momentum,
        ],
...
fn sleeve_weight(
    ...
    match portfolio {
        ...
        PortfolioKind::MomentumOnly if strategy == StrategyKind::Momentum => {
            ddhard_exposure(dd_pct, recovery_ratio)
        }
        ...
        PortfolioKind::DdBudgetAllFour => match strategy {
            ...
            | StrategyKind::MacdRegime
            | StrategyKind::SmallByDollarVol
            | StrategyKind::Momentum => 0.25 * ddhard_exposure(dd_pct, recovery_ratio),
        },
        PortfolioKind::DdBudgetAdMacdMomentum => match strategy {
            StrategyKind::AdMomentum | StrategyKind::MacdRegime | StrategyKind::Momentum => {
                (1.0 / 3.0) * ddhard_exposure(dd_pct, recovery_ratio)
            }
            _ => 0.0,
        },
        PortfolioKind::DdBudgetAdSmallMomentum => match strategy {
            StrategyKind::AdMomentum | StrategyKind::SmallByDollarVol | StrategyKind::Momentum => {
                (1.0 / 3.0) * ddhard_exposure(dd_pct, recovery_ratio)
            }
            _ => 0.0,
        },
        PortfolioKind::DdBudgetMacdSmallMomentum => match strategy {
            StrategyKind::MacdRegime | StrategyKind::SmallByDollarVol | StrategyKind::Momentum => {
                (1.0 / 3.0) * ddhard_exposure(dd_pct, recovery_ratio)
            }
            _ => 0.0,
        },
...
fn write_snapshot(universe_summaries: &[UniverseSummary]) -> Result<(String, String)> {
    ...
    let archive_md = format!("{}/factor_momentum_sleeve_audit_{}.md", SNAPSHOT_DIR, ts);
    let archive_csv = format!("{}/factor_momentum_sleeve_audit_{}.csv", SNAPSHOT_DIR, ts);
