//! Cluster-residual momentum breadth benchmark.
//!
//! Goal:
...
//!   their own calibrated peer-cluster baseline
...
impl StrategyKind {
    fn name(&self) -> &'static str {
        match self {
            Self::ClusterResidualMomentum => "ClusterResidualMomentum",
...
async fn main() -> Result<()> {
    println!("=== CLUSTER RESIDUAL MOMENTUM BREADTH BENCHMARK ===\n");
    ...
    println!("\nInterpretation:");
    println!("- ClusterResidualMomentum asks whether symbols keep trending after outperforming or underperforming their own stable peer cluster.");
...
fn generate_all_signals(
    ...
    out.insert(
        StrategyKind::ClusterResidualMomentum,
...
fn write_snapshot_files(
    ...
    fs::write(
        format!("snapshots/cluster_residual_momentum_benchmark_{}.md", ts),
        &md,
    )?;
    fs::write(
        format!("snapshots/cluster_residual_momentum_benchmark_{}.csv", ts),
        &csv,
    )?;
    Ok(())
}
