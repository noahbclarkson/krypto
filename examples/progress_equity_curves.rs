...
async fn main() -> Result<()> {
    ...
    let turtle_daily_rank24 = simulate_turtle_chandelier_equity(&universe, &btc_close, &btc_high, &btc_low, true)?;;
    ...
    // Export ATR_RANK=24 variant CSV
    ...
    for day in 0..=universe.steps {
        csv2.push_str(&format!(
            ...
            small_daily[day],
            ddbudget_daily[day],
            turtle_daily_rank24[day],
        ));
    }
    fs::write("snapshots/progress_equity_curves_atrrank24.csv", &csv2)?;
    eprintln!("CSV (ATR_RANK=24 variant) written to snapshots/progress_equity_curves_atrrank24.csv");
    ...
    let final_turtle_rank24 = turtle_daily_rank24.last().copied().unwrap_or(1.0);
    ...
    println!(
        "  Turtle ATR_RANK=24:  {:.1}x ({:.1}%)",
        final_turtle_rank24,
        (final_turtle_rank24 - 1.0) * 100.0
    );
    println!("  [MACD+Regime & Blend excluded: 2/7 OOS — GRAVEYARD]");
    ...
    let turtle_sharpe_rank24 = calc_sharpe_from_daily(&turtle_daily_rank24);
    ...
    println!("  Turtle ATR_RANK=24:  {:.2}", turtle_sharpe_rank24);
    println!("  [MACD+Regime & Blend excluded: 2/7 OOS — GRAVEYARD]");
    ...
    let md = format!(
        ...
        final_turtle_rank24,
        (final_turtle_rank24 - 1.0) * 100.0,
        turtle_sharpe_rank24,
    );
    fs::write(OUTPUT_MD, &md)?;
    println!("\nMarkdown summary written to {}", OUTPUT_MD);

    // Write markdown for ATR_RANK=24 variant
    let md2 = format!(
        "# Progress Equity Curves — ATR_RANK=24 Variant\n\nGenerated: {}\n\n\
         ...
         Final equity | Reported Sharpe:\n\
         - Turtle+ATR_RANK=24: {:.1}x ({:.1}%), Sharpe {:.2} [daily compounded equity]\n\
    ...
    fs::write("snapshots/progress_equity_curves_atrrank24.md", &md2)?;
    println!("Markdown variant written to snapshots/progress_equity_curves_atrrank24.md");
