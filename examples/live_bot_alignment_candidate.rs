//! T69: Live-Bot Semantic Alignment Candidate Harness
//!
//! Purpose: test the candidate alignment of live semantics to the research harness without promoting it to production.
//!
//! This harness intentionally models the T69 candidate semantics:
//! - per-symbol event loop in configured symbol order
//! - dollar-volume ranking before entry using VOL_LOOKBACK and POSITION_CAP
//! - Turtle entry check uses strict prior-window breakout semantics
...
//!
//! Outputs:
//! - snapshots/live_bot_alignment_candidate_equity.csv
//! - snapshots/live_bot_alignment_candidate_trades.csv
//! - snapshots/live_bot_alignment_candidate.md
...
/// Mirrors check_turtle_entry() in bot.rs after T69: strict breakout above the
/// max close of the prior EP bars. The current bar is never included in the
/// breakout threshold and equality is not sufficient.
fn live_bot_entry_signal(sd: &SymData, idx: usize) -> bool {
    if idx < TURTLE_EP {
        return false;
    }
    let ws = idx - TURTLE_EP;
    ...
    if ATR_ENTRY_MULT > 0.0 && idx + 1 >= TURTLE_ATR_PERIOD + 1 {
...
async fn main() -> Result<()> {
    println!("=== T69: LIVE-BOT SEMANTIC ALIGNMENT CANDIDATE ===");
    ...
    File::create("snapshots/live_bot_alignment_candidate_equity.csv")?
    ...
    File::create("snapshots/live_bot_alignment_candidate_trades.csv")?
        .write_all(trade_csv.as_bytes())?;

    // Markdown report.
    let mut md = File::create("snapshots/live_bot_alignment_candidate.md")?;
    writeln!(md, "# T69: Live-Bot Semantic Alignment Candidate\n")?;
    writeln!(
        md,
        "Generated: {}\n",
    ...
    writeln!(md, "## Scope\n")?;
    writeln!(md, "This harness tests a candidate live-bot semantic alignment over common timestamp-aligned Base5 symbols: strict prior-window entry + VOL_LOOKBACK dollar-volume rank gate + size-aware economic accounting. It is a deployability audit, not a new hyperopt, and does not change production bot code.\n")?;
    writeln!(md, "## Methodology / Exactness Notes\n")?;
    writeln!(md, "- Entry: strict prior-window Turtle breakout (`close > max close of prior EP bars`; current bar excluded, equality rejected).")?;
    ...
    writeln!(md, "- Accounting: economic mark-to-market account equity; candidate accounting records trade PnL and fees size-aware; production code is unchanged unless separately promoted.\n")?;
    ...
    println!("CSV: snapshots/live_bot_alignment_candidate_equity.csv");
    println!("Trades: snapshots/live_bot_alignment_candidate_trades.csv");
    println!("Report: snapshots/live_bot_alignment_candidate.md");
