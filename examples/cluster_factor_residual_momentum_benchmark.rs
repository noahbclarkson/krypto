//! Cluster-factor residual momentum breadth benchmark.
//!
//! Goal:
//! - test a more structural Track C family after the simple peer-cluster and
//!   two-cluster mean-residual rules both failed
//! - ask whether symbols keep trending after outperforming or underperforming
//!   a latent sector-style factor after removing both BTC market mode and a
//!   same-cluster factor, rather than only subtracting a raw peer average
...
//! - estimate a rolling BTC beta and a rolling cluster-factor beta for each symbol
//! - compute a 21-bar residual momentum score after removing both the BTC move and
//!   the symbol's same-cluster factor exposure
...
impl StrategyKind {
    fn name(&self) -> &'static str {
        match self {
            Self::ClusterFactorResidualMomentum => "ClusterFactorResidualMomentum",
    ...
    fn all() -> &'static [StrategyKind] {
        &[
            Self::ClusterFactorResidualMomentum,
...
async fn main() -> Result<()> {
    println!("=== CLUSTER-FACTOR RESIDUAL MOMENTUM BREADTH BENCHMARK ===\n");
    ...
    println!("\nInterpretation:");
    println!("- ClusterFactorResidualMomentum asks whether residual momentum survives after removing both BTC market mode and a same-cluster factor.");
    println!("- If it fails, that is still useful: it means simple daily relational breadth is not rescued by a slightly richer factor subtraction.");
    println!("- If it survives selectively, that points toward richer graph / latent-factor models rather than raw peer means.");
...
fn generate_all_signals(
    ...
    out.insert(
        StrategyKind::ClusterFactorResidualMomentum,
        generate_cluster_factor_residual_signals(data_cache, symbols)?,
    );
...
fn generate_cluster_factor_residual_signals(
    data_cache: &HashMap<String, DataFrame>,
    symbols: &[&str],
    ...
    let mut cluster_factor_map = HashMap::<String, Vec<f64>>::new();
    for &symbol in symbols {
        ...
        for i in 0..n {
            series[i] =
                cluster_factor_return(&close_map, &peers, i, RESIDUAL_LOOKBACK).unwrap_or(f64::NAN);
        }
        cluster_factor_map.insert(symbol.to_string(), series);
    }
        ...
        WARMUP_BARS.max(CALIBRATION_BARS).max(RESIDUAL_LOOKBACK)..(n.saturating_sub(HOLD_BARS + 1))
    {
        ...
        for &symbol in symbols {
            ...
            let factor_series = cluster_factor_map.get(symbol).unwrap();
            let residual = cluster_factor_residual_return(
                close,
                &btc_close,
                factor_series,
                i,
...
fn cluster_factor_residual_return(
    asset: &[f64],
    btc: &[f64],
    cluster_factor: &[f64],
    i: usize,
...
fn write_snapshot_files(
    ...
    fs::write(
        format!(
            "snapshots/cluster_factor_residual_momentum_benchmark_{}.md",
            ts
        ),
        &md,
    )?;
    fs::write(
        format!(
            "snapshots/cluster_factor_residual_momentum_benchmark_{}.csv",
            ts
        ),
        &csv,
    )?;
