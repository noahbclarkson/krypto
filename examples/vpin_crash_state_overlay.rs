//! VPIN-style crash-state overlay for the frozen three-sleeve reference book.
//!
//! Purpose:
//! - answer the still-open PLAN item directly: test a genuinely new microstructure-flavoured
//!   crash-state lane instead of drifting into another overlay calibration loop
//! - build a daily-bar VPIN-style toxicity proxy from BTC volume imbalance and range position
//! - judge the state only as allocator context on top of the frozen
...
const TOXICITY_LOOKBACK: usize = 21;
const CONN_LOOKBACK: usize = 63;
const VOL_LOOKBACK: usize = 21;
const BREADTH_LOOKBACK: usize = 63;
const SNAPSHOT_DIR: &str = "snapshots";
const SNAPSHOT_LATEST_MD: &str = "snapshots/vpin_crash_state_overlay_latest.md";
const SNAPSHOT_LATEST_CSV: &str = "snapshots/vpin_crash_state_overlay_latest.csv";
...
impl PortfolioKind {
    fn all() -> &'static [PortfolioKind] {
        &[Self::DdBudgetBaseline, Self::Vpintilt, Self::VpinRiskOff]
    }

    fn name(&self) -> &'static str {
        match self {
            Self::DdBudgetBaseline => "DDBudget(A/D,MACD,Small)",
            Self::Vpintilt => "VPINTilt(A/D,MACD,Small)",
            Self::VpinRiskOff => "VPINRiskOff(A/D,MACD,Small)",
        }
    }
}
...
async fn main() -> Result<()> {
    println!("=== VPIN CRASH-STATE OVERLAY ===\n");
    println!("Question: can a daily VPIN-style crash-state improve the frozen three-sleeve book without another local sleeve loop?");
    println!("Baseline: DDBudget(A/D,MACD,Small) as the current risk-adjusted reference book.");
    println!(
        "State inputs: BTC/ETH daily VPIN-style toxicity proxy plus BTC vol shock confirmation."
    );
    println!(
    ...
    println!(
        "Honesty note: this is a daily-bar VPIN-style toxicity proxy, not true trade-level VPIN.\n"
    );
    ...
    for &(label, universe_symbols) in UNIVERSES {
        ...
        let state = build_break_state(&universe, &bench_df, &sleeves)?;
        ...
        for s in &state {
            match s {
                BreakState::Healthy => state_counts[0] += 1,
                BreakState::Neutral => state_counts[1] += 1,
                BreakState::Stressed => state_counts[2] += 1,
            }
        }
...
fn build_break_state(
    universe: &UniverseData,
    bench_df: &DataFrame,
    sleeves: &[FamilySleeve],
) -> Result<Vec<BreakState>> {
    let steps = universe.steps;
    let bench_close = series_to_vec(bench_df.column("close")?.f64()?);
    let bench_open = series_to_vec(bench_df.column("open")?.f64()?);
    let bench_high = series_to_vec(bench_df.column("high")?.f64()?);
    let bench_low = series_to_vec(bench_df.column("low")?.f64()?);
    let bench_volume = series_to_vec(bench_df.column("volume")?.f64()?);

    let btc_vol = rolling_realized_vol(&bench_close, VOL_LOOKBACK, steps);
    let breadth = sleeve_breadth_metric(sleeves, steps);
    let toxicity = build_vpin_toxicity_metric(
        &bench_open,
        &bench_high,
        &bench_low,
        &bench_close,
    ...
    let tox_z = zscore_series(&toxicity, TOXICITY_LOOKBACK);
    let vol_z = zscore_series(&btc_vol, VOL_LOOKBACK);
    let breadth_z = zscore_series(&breadth, BREADTH_LOOKBACK);
    let tox_delta_z = zscore_series(&first_difference(&toxicity), TOXICITY_LOOKBACK);
    let breadth_delta_z = zscore_series(&first_difference(&breadth), BREADTH_LOOKBACK);

    let mut score = vec![0.0; steps + 1];
    for day in 0..=steps {
        let toxicity_break = tox_z[day].max(0.0) + 0.75 * tox_delta_z[day].max(0.0);
        let vol_break = vol_z[day].max(0.0);
        let breadth_break = (-breadth_z[day]).max(0.0) + 0.50 * (-breadth_delta_z[day]).max(0.0);
        score[day] = 0.60 * toxicity_break + 0.25 * vol_break + 0.25 * breadth_break;
    }
    ...
    let std = var.max(1e-12).sqrt();
    let stressed_cut = mean + 0.80 * std;
    let healthy_cut = mean - 0.45 * std;

    let mut states = vec![BreakState::Neutral; steps + 1];
    for day in 0..=steps {
        states[day] = if day < WARMUP_BARS {
            BreakState::Neutral
        } else if score[day] >= stressed_cut {
            BreakState::Stressed
        } else if score[day] <= healthy_cut && tox_z[day] < 0.0 {
            BreakState::Healthy
        } else {
            BreakState::Neutral
        };
...
fn build_vpin_toxicity_metric(
    ...
    let n = open
        ...
        .min(volume.len())
        .min(steps + 1);
    let mut signed_volume = vec![0.0; steps + 1];
    let mut imbalance_ratio = vec![0.0; steps + 1];

    for i in 0..n {
        let range = (high[i] - low[i]).abs().max(1e-9);
        let body = close[i] - open[i];
        let close_loc = (2.0 * close[i] - high[i] - low[i]) / range;
        let direction = 0.6 * close_loc.clamp(-1.0, 1.0) + 0.4 * (body / range).clamp(-1.0, 1.0);
        signed_volume[i] = volume[i] * direction;
        imbalance_ratio[i] = signed_volume[i].abs() / volume[i].max(1e-9);
    }

    let abs_signed = rolling_mean(&imbalance_ratio, TOXICITY_LOOKBACK);
    let vol_baseline = rolling_mean(volume, TOXICITY_LOOKBACK);
    let imbalance_persistence = rolling_mean_abs_diff(&signed_volume, 5);
    let mut toxicity = vec![0.0; steps + 1];
    for i in 0..=steps.min(n.saturating_sub(1)) {
        let persistence_scaled = imbalance_persistence[i] / vol_baseline[i].max(1e-9);
        toxicity[i] = 0.75 * abs_signed[i] + 0.25 * persistence_scaled;
    }
    toxicity
}
...
fn rolling_mean(values: &[f64], lookback: usize) -> Vec<f64> {
    ...
    for i in 0..values.len() {
        ...
        out[i] = slice.iter().sum::<f64>() / slice.len().max(1) as f64;
    }
    out
}

fn rolling_mean_abs_diff(values: &[f64], lookback: usize) -> Vec<f64> {
    let mut out = vec![0.0; values.len()];
    for i in 1..values.len() {
        ...
        for j in (start + 1)..=i {
            sum += (values[j] - values[j - 1]).abs();
            count += 1;
        }
        out[i] = if count > 0 { sum / count as f64 } else { 0.0 };
    }
    out
}
...
fn simulate_family_portfolio(
    ...
    for day in 0..steps {
        ...
        let break_state = state.get(day).copied().unwrap_or(BreakState::Neutral);
        if break_state == BreakState::Stressed {
            stressed_days += 1;
        }
        ...
        for sleeve in sleeves {
            ...
            let weight = sleeve_weight(portfolio, sleeve.strategy, dd, rec, break_state);
...
fn sleeve_weight(
    ...
    match portfolio {
        ...
        PortfolioKind::Vpintilt => match (strategy, break_state) {
            (StrategyKind::AdMomentum, BreakState::Stressed) => {
                0.40 * ddhard_exposure(dd_pct, recovery_ratio)
            }
            (StrategyKind::MacdRegime, BreakState::Stressed) => {
                0.17 * ddhard_exposure(dd_pct, recovery_ratio)
            }
            (StrategyKind::SmallByDollarVol, BreakState::Stressed) => {
                0.23 * ddhard_exposure(dd_pct, recovery_ratio)
            }
            (StrategyKind::AdMomentum, BreakState::Healthy) => {
                0.30 * ddhard_exposure(dd_pct, recovery_ratio)
            }
            (StrategyKind::MacdRegime, BreakState::Healthy) => {
                0.36 * ddhard_exposure(dd_pct, recovery_ratio)
            }
            (StrategyKind::SmallByDollarVol, BreakState::Healthy) => {
                0.34 * ddhard_exposure(dd_pct, recovery_ratio)
            }
            _ => base,
        },
        PortfolioKind::VpinRiskOff => match (strategy, break_state) {
            (StrategyKind::AdMomentum, BreakState::Stressed) => {
                0.45 * ddhard_exposure(dd_pct, recovery_ratio)
            }
            (StrategyKind::MacdRegime, BreakState::Stressed) => {
                0.08 * ddhard_exposure(dd_pct, recovery_ratio)
            }
            (StrategyKind::SmallByDollarVol, BreakState::Stressed) => {
                0.14 * ddhard_exposure(dd_pct, recovery_ratio)
            }
            (StrategyKind::AdMomentum, BreakState::Healthy) => {
                0.29 * ddhard_exposure(dd_pct, recovery_ratio)
            }
            (StrategyKind::MacdRegime, BreakState::Healthy) => {
                0.39 * ddhard_exposure(dd_pct, recovery_ratio)
            }
            (StrategyKind::SmallByDollarVol, BreakState::Healthy) => {
                0.32 * ddhard_exposure(dd_pct, recovery_ratio)
            }
            _ => base,
        },
    }
...
fn write_snapshot(summaries: &[UniverseSummary]) -> Result<(String, String)> {
    ...
    let archive_md = format!("{}/vpin_crash_state_overlay_{}.md", SNAPSHOT_DIR, ts);
    let archive_csv = format!("{}/vpin_crash_state_overlay_{}.csv", SNAPSHOT_DIR, ts);
