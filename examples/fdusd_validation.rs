...
use krypto::{
    ...
    backtest::{
        engine::{BacktestResult, Backtester, PositionSizing},
...
async fn main() -> Result<()> {
    ...
    println!(
        "{}",
        "  FDUSD PASSIVE EXECUTION VALIDATION".bright_cyan().bold()
    );
    ...
    println!(
        "  Lower TF:  {} (adaptive, {} bars/signal bar)",
    ...
    for cand in CANDIDATES {
        ...
        let usdt_result = match loader.fetch_data(&usdt_sym, INTERVAL, CANDLES).await {
            Ok(df) => {
                ...
                match bt.run(&df, &signals, stop, 0.0) {
                    ...
                    Err(e) => {
                        println!("    USDT error: {}", e);
                        None
                    }
                }
            }
            Err(e) => {
                println!("    USDT load failed: {}", e);
                None
            }
        };
        ...
        let fdusd_result = match loader.fetch_data(&fdusd_sym, INTERVAL, FDUSD_CANDLES).await {
            Ok(df_high) => {
                ...
                let bt =
                    Backtester::new(CAPITAL, 0.0, 0.0).with_position_sizing(PositionSizing::Full);

                match bt
                    .run_with_passive(&df_high, &df_low, &signals, stop, 0.0, passive_cfg.clone())
                    .await
                {
                    Ok((r, stats)) => {
                    ...
                    Err(e) => {
                        println!("    FDUSD passive error: {}", e);
                        None
                    }
                }
            }
            Err(e) => {
                println!("    FDUSD load failed (pair may not exist): {}", e);
                None
            }
        };
    ...
    println!(
        "{}",
        "  SUMMARY: USDT TAKER vs FDUSD PASSIVE"
            .bright_cyan()
            .bold()
    );
    ...
    for row in &rows {
        let usdt_ann = row
            .usdt_result
            .as_ref()
            .map(|r| r.annualised_return_pct)
            .unwrap_or(f64::NAN);
        let usdt_shp = row
            .usdt_result
            .as_ref()
            .map(|r| r.annualised_sharpe)
            .unwrap_or(f64::NAN);
        let fds_ann = row
            .fdusd_result
            .as_ref()
    ...
    if shp_delta_count > 0 {
        ...
        println!(
            "\n  Avg Sharpe delta (FDUSD passive − USDT taker): {:>+.2}",
            avg_delta
        );
        if avg_delta > 0.0 {
            println!(
                "  {} FDUSD passive outperforms USDT taker on Sharpe",
                "✓".green()
            );
        } else {
            println!(
                "  {} USDT taker outperforms FDUSD passive on Sharpe",
                "✗".red()
            );
        }
    }
    ...
    println!(
        "{}",
        "  VIABLE FDUSD CANDIDATES (fill rate ≥ 50%, ann. return > 0%)"
            .bright_cyan()
            .bold()
    );
    println!("{}", "━".repeat(70).bright_cyan());

    let viable: Vec<_> = rows
        .iter()
        .filter(|r| {
            r.fdusd_fill_rate >= 0.5
                && r.fdusd_result
                    .as_ref()
                    .map(|res| res.annualised_return_pct > 0.0)
                    .unwrap_or(false)
                && r.fdusd_result
                    .as_ref()
                    .map(|res| res.total_trades >= 15)
                    .unwrap_or(false)
        })
        .collect();
