//! Observable change-point / structural-break overlay for the frozen three-sleeve reference book.
//!
//! Purpose:
//! - answer the still-open PLAN item directly: build an explicit change-point state instead of
...
const SNAPSHOT_LATEST_MD: &str = "snapshots/change_point_state_overlay_latest.md";
const SNAPSHOT_LATEST_CSV: &str = "snapshots/change_point_state_overlay_latest.csv";
...
impl PortfolioKind {
    fn all() -> &'static [PortfolioKind] {
        &[Self::DdBudgetBaseline, Self::BreakTilt, Self::BreakRiskOff]
    }

    fn name(&self) -> &'static str {
        match self {
            Self::DdBudgetBaseline => "DDBudget(A/D,MACD,Small)",
            Self::BreakTilt => "BreakTilt(A/D,MACD,Small)",
            Self::BreakRiskOff => "BreakRiskOff(A/D,MACD,Small)",
        }
    }
}
...
async fn main() -> Result<()> {
    println!("=== CHANGE-POINT STATE OVERLAY ===\n");
    println!("Question: can a simple change-point state improve the frozen three-sleeve book without another local sleeve loop?");
    println!("Baseline: DDBudget(A/D,MACD,Small) as the current risk-adjusted reference book.");
    println!("State inputs: breadth break, BTC vol shock, and correlation concentration / change.");
    println!(
    ...
    println!(
        "Honesty note: this is a hand-built change-point state map, not a hidden-state model.\n"
    );
    ...
    for &(label, universe_symbols) in UNIVERSES {
        ...
        let state = build_break_state(&universe, &bench_close, &sleeves)?;
...
fn build_break_state(
    ...
    let conn_delta_z = zscore_series(&first_difference(&connectedness), CONN_LOOKBACK);
    let breadth_delta_z = zscore_series(&first_difference(&breadth), BREADTH_LOOKBACK);

    let mut score = vec![0.0; steps + 1];
    for day in 0..=steps {
        let corr_break = conn_z[day].max(0.0) + 0.75 * conn_delta_z[day].max(0.0);
        let vol_break = vol_z[day].max(0.0);
        let breadth_break = (-breadth_z[day]).max(0.0) + 0.75 * (-breadth_delta_z[day]).max(0.0);
        score[day] = 0.40 * corr_break + 0.30 * vol_break + 0.45 * breadth_break;
    }
    ...
    let std = var.max(1e-12).sqrt();
    let stressed_cut = mean + 0.35 * std;
    let healthy_cut = mean - 0.25 * std;
...
fn sleeve_weight(
    ...
    match portfolio {
        ...
        PortfolioKind::BreakTilt => match (strategy, break_state) {
        ...
        PortfolioKind::BreakRiskOff => match (strategy, break_state) {
            (StrategyKind::AdMomentum, BreakState::Stressed) => {
...
fn write_snapshot(summaries: &[UniverseSummary]) -> Result<(String, String)> {
    ...
    let archive_md = format!("{}/change_point_state_overlay_{}.md", SNAPSHOT_DIR, ts);
    let archive_csv = format!("{}/change_point_state_overlay_{}.csv", SNAPSHOT_DIR, ts);
