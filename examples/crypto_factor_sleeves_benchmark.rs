//! Crypto factor-sleeve benchmark under the same harsh-universe lens.
//!
//! Purpose:
//! - deliberately open a broader Track C lane after many sessions around the same
//!   A/D + trend + allocator neighborhood
//! - test simple, pre-declared crypto factor sleeves using only accessible daily data
//! - compare them against current yardsticks without pretending they are deployable
//!
//! Factor sleeves (all intentionally simple):
//! - Momentum: multi-horizon price trend rank
//! - SmallByDollarVol: inverse rolling dollar-volume rank (small/liquidity proxy, not true market cap)
//! - Illiquidity: rolling Amihud-style illiquidity rank
//! - Small+Momentum: require small/liquidity tilt to agree with momentum
//! - Illiquid+Momentum: require illiquidity tilt to agree with momentum
//!
//! Important honesty note:
//! - crypto spot market cap history is not available in the current local lab
//! - so "size" here means a tradability / dollar-volume proxy, not true market cap
//! - this is a factor-audit result, not a promotion result
...
impl StrategyKind {
    ...
    fn name(&self) -> &'static str {
        match self {
            ...
            Self::Illiquidity => "FactorIlliquidity",
            Self::SmallMomentum => "FactorSmall+Momentum",
            Self::IlliquidMomentum => "FactorIlliquid+Momentum",
    ...
    fn description(&self) -> &'static str {
        match self {
            Self::Momentum => "cross-sectional multi-horizon price strength",
            Self::SmallByDollarVol => "small/liquidity proxy via inverse dollar volume",
            Self::Illiquidity => "Amihud-style illiquidity proxy",
            Self::SmallMomentum => "small proxy only when momentum agrees",
            Self::IlliquidMomentum => "illiquidity only when momentum agrees",
...
async fn main() -> Result<()> {
    println!("=== CRYPTO FACTOR SLEEVES BENCHMARK ===\n");
    ...
    println!("Honesty note: size = inverse dollar-volume proxy, not true market cap.\n");
    ...
    println!("\nInterpretation:");
    println!("- This is a search-broadening factor audit, not a deployment decision.");
    println!("- If simple factor sleeves cannot survive quarter/resample stress here, they are not yet worthy of allocator attention.");
    println!(
        "- If they do survive, they earn follow-up as separate sleeves, not immediate promotion."
    );
...
fn build_universe_signals(
    ...
    out.insert(
        ...
        generate_momentum_factor_signals(data_cache, symbols)?,
    );
    out.insert(
    ...
    out.insert(
        StrategyKind::Illiquidity,
        generate_illiquidity_signals(data_cache, symbols)?,
    );
    out.insert(
        StrategyKind::SmallMomentum,
        combine_agreeing(
            out.get(&StrategyKind::Momentum).unwrap(),
            out.get(&StrategyKind::SmallByDollarVol).unwrap(),
        )?,
    );
    out.insert(
        StrategyKind::IlliquidMomentum,
        combine_agreeing(
            out.get(&StrategyKind::Momentum).unwrap(),
            out.get(&StrategyKind::Illiquidity).unwrap(),
        )?,
    );
...
fn generate_momentum_factor_signals(
    data_cache: &HashMap<String, DataFrame>,
    symbols: &[&str],
) -> Result<HashMap<String, Vec<i32>>> {
...
fn generate_illiquidity_signals(
    data_cache: &HashMap<String, DataFrame>,
    symbols: &[&str],
) -> Result<HashMap<String, Vec<i32>>> {
    let n = min_rows(data_cache, symbols)?;
    ...
    for &symbol in symbols {
        ...
        let close = df.column("close")?.f64()?;
        let volume = df.column("volume")?.f64()?;
        let mut score = rolling_amihud(close, volume, 21);
        score.truncate(n);
        raw_scores.insert(symbol.to_string(), score);
    }
    rank_to_signals(&raw_scores, 0.35)
}
