...
impl StrategyKind {
    ...
    fn name(&self) -> &'static str {
        match self {
            Self::WeeklyPressure => "WeeklyPressure",
    ...
    fn description(&self) -> &'static str {
        match self {
            Self::WeeklyPressure => "5-21d signed dollar-flow pressure proxy",
            Self::WeeklyPressureAccel => "weekly pressure with short-horizon acceleration",
            Self::WeeklyPressureSmall => "pressure sleeve only when small/liquidity tilt agrees",
...
async fn main() -> Result<()> {
    println!("=== WEEKLY PRESSURE FACTOR BENCHMARK ===\n");
    ...
    println!("Honesty note: pressure here is OHLCV-derived weekly flow pressure, not true order-book or taker-flow data.\n");
...
fn build_universe_signals(
    ...
    out.insert(
        StrategyKind::WeeklyPressure,
        generate_weekly_pressure_signals(data_cache, symbols)?,
    );
    out.insert(
    ...
    out.insert(
        ...
        combine_agreeing(
            out.get(&StrategyKind::WeeklyPressure).unwrap(),
...
fn generate_weekly_pressure_signals(
    data_cache: &HashMap<String, DataFrame>,
    symbols: &[&str],
) -> Result<HashMap<String, Vec<i32>>> {
    ...
    for &symbol in symbols {
        ...
        for i in 63..n {
            let scale_fast = dv_21[i].max(1.0);
            let scale_slow = dv_63[i].max(1.0);
            scores[i] = 0.50 * (flow_5[i] / scale_fast)
                + 0.30 * (flow_10[i] / scale_fast)
                + 0.20 * (flow_21[i] / scale_slow);
        }
