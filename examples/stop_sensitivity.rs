...
async fn main() -> Result<()> {
        ...
        for interval in INTERVALS {
            ...
            for (name, make_strat) in &strategies {
                ...
                for &stop in STOP_SIZES {
                    ...
                    if result.total_trades < 15 { continue; }
    ...
    for &stop in STOP_SIZES {
        let stop_rows: Vec<_> = rows.iter().filter(|r| (r.stop_pct - stop * 100.0).abs() < 0.01).collect();
        if stop_rows.is_empty() { continue; }
    ...
    let best_stop = STOP_SIZES.iter().max_by(|a, b| {
        let avg_a: f64 = rows.iter().filter(|r| (r.stop_pct - **a * 100.0).abs() < 0.01)
            .map(|r| r.return_pct).sum::<f64>();
        let avg_b: f64 = rows.iter().filter(|r| (r.stop_pct - **b * 100.0).abs() < 0.01)
            .map(|r| r.return_pct).sum::<f64>();
