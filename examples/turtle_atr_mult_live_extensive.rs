//! TURTLE_ATR_MULT live-path extensive hyperopt.
//!
//! Parameter audited: `TURTLE_ATR_MULT` in `src/live/config.rs`.
//! Current default is hardcoded to 2.0; prior sweep was coarse/dual-exit, not
//! exact-live-style Turtle-only path with ATR_RANK(AP=17, LB=41, T=5).
//!
//! Sweep: multiplier = 0.50..=5.00, step 0.05 (91 values).
...
const BASELINE_TURTLE_ATR_MULT: f64 = 2.0;
...
const MULT_MIN_BPS: usize = 50; // 0.50
const MULT_MAX_BPS: usize = 500; // 5.00
const MULT_STEP_BPS: usize = 5; // 0.05
const FRESHNESS_COOLDOWN: usize = 0;

const SUMMARY_OUT: &str = "snapshots/turtle_atr_mult_live_summary.csv";
const WINDOWS_OUT: &str = "snapshots/turtle_atr_mult_live_windows.csv";
const EQUITY_OUT: &str = "snapshots/turtle_atr_mult_live_equity.csv";
const SUMMARY_JSON_OUT: &str = "snapshots/turtle_atr_mult_live_summary.json";
            ...
            if let Some(mut pos) = positions.remove(sym) {
                if idx > pos.entry_bar {
                    ...
                    if pos.bars_held >= HOLD_MAX {
                        ...
                        let stop = pos.highest_high - atr_mult * atr;
                        if sd.low[idx] <= stop {
                            exit = true;
                        }
                    }
...
async fn main() -> Result<()> {
    println!("=== TURTLE_ATR_MULT LIVE-PATH EXTENSIVE SWEEP ===");
    println!(
        "Sweep mult={:.2}..={:.2} step {:.2}",
        MULT_MIN_BPS as f64 / 100.0,
        MULT_MAX_BPS as f64 / 100.0,
        MULT_STEP_BPS as f64 / 100.0
    );
    println!("Params: EP={}, ATR_P={}, baseline ATR_M={:.2}, HM={}, CAP={}, ATR_RANK(AP={}, LB={}, T={:.1}), FRESHNESS_COOLDOWN={}", TURTLE_EP, TURTLE_ATR_PERIOD, BASELINE_TURTLE_ATR_MULT, HOLD_MAX, POSITION_CAP, REGIME_ATR_PERIOD, REGIME_LOOKBACK, ATR_RANK_THRESHOLD, FRESHNESS_COOLDOWN);
    ...
    writeln!(window_csv, "atr_mult,universe,window,final_equity,return_pct,sharpe,max_dd_pct,trades,win_rate_pct,pass")?;
    ...
    for mult_bps in (MULT_MIN_BPS..=MULT_MAX_BPS).step_by(MULT_STEP_BPS) {
        let atr_mult = mult_bps as f64 / 100.0;
        for (u_name, u_syms) in UNIVERSES {
            ...
            for w in 0..windows {
                ...
                let res = run_sim(&data, &symbols, test_start, test_end, atr_mult);
                ...
                let a = agg.entry(mult_bps).or_default();
    ...
    writeln!(summary_csv, "atr_mult,pass_count,total_windows,pass_rate_pct,avg_sharpe,avg_return_pct,avg_max_dd_pct,total_trades,win_rate_pct")?;
    let mut rows = Vec::new();
    for mult_bps in (MULT_MIN_BPS..=MULT_MAX_BPS).step_by(MULT_STEP_BPS) {
        let atr_mult = mult_bps as f64 / 100.0;
        let a = agg.get(&mult_bps).cloned().unwrap_or_default();
        ...
        writeln!(
            summary_csv,
            "{:.2},{},{},{:.4},{:.6},{:.4},{:.4},{},{:.4}",
            atr_mult, a.pass, a.total, pass_rate, avg_sharpe, avg_ret, avg_dd, a.trades, win_rate
        )?;
        rows.push((
            mult_bps, a.pass, a.total, pass_rate, avg_sharpe, avg_ret, avg_dd, a.trades, win_rate,
        ));
    }
    ...
    println!("\nTop 10 multipliers by robustness (pass rate, then Sharpe):");
    for (mult_bps, pass, total, pr, sh, ret, dd, trades, wr) in rows.iter().take(10) {
        println!("M={:4.2} | pass {:2}/{:<2} ({:5.1}%) | Sharpe {:6.3} | Ret {:7.2}% | DD {:5.2}% | trades {:4} | WR {:5.1}%", *mult_bps as f64 / 100.0, pass, total, pr, sh, ret, dd, trades, wr);
    }

    let mut json = File::create(SUMMARY_JSON_OUT)?;
    writeln!(json, "{{")?;
    writeln!(json, "  \"parameter\": \"TURTLE_ATR_MULT\",")?;
    writeln!(json, "  \"baseline\": {:.2},", BASELINE_TURTLE_ATR_MULT)?;
    if let Some((winner_bps, _, _, _, _, _, _, _, _)) = rows.first() {
        writeln!(json, "  \"winner\": {:.2},", *winner_bps as f64 / 100.0)?;
    }
    writeln!(
        json,
    ...
    writeln!(
        json,
        "  \"values_tested\": {}",
        ((MULT_MAX_BPS - MULT_MIN_BPS) / MULT_STEP_BPS) + 1
    )?;
    writeln!(json, "}}")?;

    // Export Base5 full-history equity curves for every multiplier. The chart script
    ...
    writeln!(eq_csv, "atr_mult,step,equity")?;
    for mult_bps in (MULT_MIN_BPS..=MULT_MAX_BPS).step_by(MULT_STEP_BPS) {
        let atr_mult = mult_bps as f64 / 100.0;
        let res = run_sim(&data, &base5_symbols, WARMUP_BARS, base5_min_len, atr_mult);
        for (step, eq) in res.equity_curve.iter().enumerate() {
            writeln!(eq_csv, "{:.2},{},{:.8}", atr_mult, step, eq)?;
        }
    }
