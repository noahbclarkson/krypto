//! Decompose worst capped-book drawdowns for the current realistic yardsticks.
//!
//! Purpose:
...
//! - extend drawdown attribution beyond the two trend rows to include the majority ensemble
...
const CHART_PATH: &str = "charts/cross_family_drawdown_attribution.png";
const SNAPSHOT_LATEST_MD: &str = "snapshots/cross_family_drawdown_attribution_latest.md";
const SNAPSHOT_LATEST_CSV: &str = "snapshots/cross_family_drawdown_attribution_latest.csv";
...
fn main() -> Result<()> {
    println!("=== CROSS-FAMILY CAPPED-BOOK DRAWDOWN ATTRIBUTION ===\n");
    println!(
        "Goal: identify which symbols drive the worst capped-book drawdown for the current realistic yardsticks"
    );
    println!(
    ...
    for (label, symbols) in cases {
        ...
        for &strategy in StrategyKind::all() {
            ...
            println!(
                "- {:<24} worst DD {:>6.1}% | peak {} -> trough {} | window return {:+.1}%",
                strategy.name(),
...
fn write_markdown(summaries: &[DrawdownSummary], path: &str) -> Result<()> {
    ...
    out.push_str("# Cross-family capped-book drawdown attribution\n\n");
...
fn write_csv(summaries: &[DrawdownSummary], path: &str) -> Result<()> {
    let mut out = String::from(
        "universe,strategy,peak_idx,trough_idx,worst_drawdown_pct,window_return_pct,symbol,contribution_pct\n",
    );
    for summary in summaries {
        for row in &summary.contributions {
            out.push_str(&format!(
...
fn draw_attribution_chart(summaries: &[DrawdownSummary], path: &str) -> Result<()> {
    let root = BitMapBackend::new(path, (2200, 1500)).into_drawing_area();
    root.fill(&WHITE)?;
...
fn generate_signals(df: &DataFrame, strategy: StrategyKind) -> Result<Vec<i32>> {
    ...
    let csm_scores = CrossSectionalMomentum::new().predict(df)?;
    let csm = series_to_i32(&csm_scores);
    Ok(match strategy {
        StrategyKind::MacdRegime => macd_regime,
        StrategyKind::TurtleRegimeMacd => turtle_regime_macd,
        StrategyKind::EnsembleMajority3 => {
            consensus_signal(&[&macd_regime, &turtle_regime_macd, &csm], 2)
        }
    })
}
...
fn generate_strengths(df: &DataFrame, strategy: StrategyKind) -> Result<Vec<f64>> {
    ...
    let csm_scores = CrossSectionalMomentum::new().predict(df)?;
    let csm_strength = csm_scores
        .f64()?
        .into_iter()
        .map(|v| v.unwrap_or(0.0).abs())
        .collect::<Vec<_>>();

    Ok(match strategy {
        StrategyKind::MacdRegime => macd_strength,
        StrategyKind::TurtleRegimeMacd => turtle_strength,
        StrategyKind::EnsembleMajority3 => {
            combine_majority_strengths(df, &macd_strength, &turtle_strength, &csm_strength)?
        }
    })
}

fn combine_majority_strengths(
    df: &DataFrame,
    macd_strength: &[f64],
    turtle_strength: &[f64],
    csm_strength: &[f64],
    ...
    let macd_regime = generate_macd_regime_signals(df)?;
    let turtle_regime_macd = generate_turtle_regime_macd_signals(df, TURTLE_PERIOD)?;
    let csm = series_to_i32(&CrossSectionalMomentum::new().predict(df)?);
    let mut out = vec![0.0; df.height()];

    for i in 0..out.len() {
        let long_votes = (macd_regime[i] > 0) as usize
            + (turtle_regime_macd[i] > 0) as usize
            + (csm[i] > 0) as usize;
        let short_votes = (macd_regime[i] < 0) as usize
            + (turtle_regime_macd[i] < 0) as usize
            + (csm[i] < 0) as usize;
        if long_votes >= 2 || short_votes >= 2 {
            out[i] = macd_strength[i] + turtle_strength[i] + csm_strength[i];
        }
    }
    Ok(out)
}
