//! Residual-momentum breadth benchmark.
//!
//! Goal:
//! - test an under-served Track C family that is different from the recent
//!   rolling panel-model trust loop
//! - ask whether cross-sectional momentum still survives after stripping out a
//!   simple rolling BTC market beta
//! - keep the same fair daily harness used elsewhere so results stay comparable
//!
//! Construction:
//! - estimate each symbol's rolling BTC beta from prior close-close returns
//! - compute lookback return residual = symbol return - beta * BTC return
//! - rank symbols each day on that residual momentum signal
//! - long the strongest positive residuals, short the weakest negative residuals
//!
...
impl StrategyKind {
    fn name(&self) -> &'static str {
        match self {
            Self::ResidualMomentum => "ResidualMomentum(BTC beta-adj)",
...
async fn main() -> Result<()> {
    println!("=== RESIDUAL MOMENTUM BREADTH BENCHMARK ===\n");
    println!("Benchmark beta anchor: {}", BENCHMARK);
    println!("Beta lookback: {} bars", BETA_LOOKBACK);
    println!("Residual momentum lookback: {} bars", MOM_LOOKBACK);
    println!(
    ...
    println!("\nInterpretation:");
    println!("- ResidualMomentum asks whether cross-sectional strength survives after removing a simple rolling BTC beta component.");
    println!("- If it fails, that is still useful: it means much of the prior cross-sectional edge was just market-mode exposure.");
    println!("- If it survives selectively on harsher baskets, that points toward a cleaner relative-value family worth deeper follow-up.");
...
fn generate_all_signals(
    ...
    out.insert(
        StrategyKind::ResidualMomentum,
        generate_residual_momentum_signals(data_cache, symbols)?,
    );
...
fn generate_residual_momentum_signals(
    data_cache: &HashMap<String, DataFrame>,
    symbols: &[&str],
) -> Result<HashMap<String, Vec<i32>>> {
    let n = min_rows(data_cache)?;
    let bench_close = collect_f64(data_cache.get(BENCHMARK).unwrap(), "close")?;
    let bench_ret = close_returns(&bench_close);
    ...
    for i in WARMUP_BARS.max(BETA_LOOKBACK + MOM_LOOKBACK)..(n.saturating_sub(HOLD_BARS + 1)) {
        let mut scores = Vec::<(String, f64)>::new();
        for &symbol in symbols {
            let returns = ret_map.get(symbol).unwrap();
            let close = close_map.get(symbol).unwrap();
            let beta = rolling_beta(returns, &bench_ret, i, BETA_LOOKBACK);
            let sym_mom = lookback_return(close, i, MOM_LOOKBACK);
            let bench_mom = lookback_return(&bench_close, i, MOM_LOOKBACK);
            let score = sym_mom - beta * bench_mom;
            if score.is_finite() {
...
fn rolling_beta(asset_ret: &[f64], bench_ret: &[f64], end: usize, lookback: usize) -> f64 {
    if end < lookback + 1 {
        return 0.0;
    }
    let start = end.saturating_sub(lookback);
    let a = &asset_ret[start..end];
    let b = &bench_ret[start..end];
...
fn write_snapshot_files(
    ...
    fs::write("snapshots/residual_momentum_benchmark_latest.md", &md)?;
    fs::write("snapshots/residual_momentum_benchmark_latest.csv", &csv)?;
    fs::write(
        format!("snapshots/residual_momentum_benchmark_{}.md", ts),
        &md,
    )?;
    fs::write(
        format!("snapshots/residual_momentum_benchmark_{}.csv", ts),
        &csv,
    )?;
    Ok(())
}
