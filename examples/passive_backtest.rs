...
async fn main() -> Result<()> {
    ...
    println!(
        "{}",
        "  PASSIVE BACKTEST: Limit orders + 0% FDUSD Maker Fees"
            .bright_cyan()
            .bold()
    );
    println!(
        "{}",
        "  stop=12%  tp=20%  FDUSD pairs  anchor_to_signal=true".bright_cyan()
    );
    ...
    for &sym in SYMBOLS {
        let tick = TickSize::fetch(sym)
            .await
            .unwrap_or(TickSize::from_value(0.01));
    ...
    for symbol in SYMBOLS {
        for interval in INTERVALS {
            ...
            for (name, make_strat) in &strategies {
                ...
                if r_market.total_trades < MIN_TRADES {
                    continue;
                }
