...
async fn main() -> anyhow::Result<()> {
    ...
    println!(
        "Strict gates: train_sharpe>0.05, pf>1.2, train_trades>20, OOS_trades>10, robustness>0.4"
    );
    ...
    for interval in &intervals {
        println!(
            "{}",
            format!("── Interval: {} ──", interval).yellow().bold()
        );
