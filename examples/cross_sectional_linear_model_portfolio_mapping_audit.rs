//! Portfolio-level symbol-mapping audit for the rolling pooled linear alpha model.
//!
//! Goal:
//! - extend the earlier mapping audit into the same realistic top-3 capped-book lens used for the
//!   incumbent yardsticks
//! - ask the sharper remaining trust question: does the richer-model family still survive because
//!   the score belongs on the right symbol, or does too much edge remain once portfolio
//!   construction / concentration is realistic?
...
impl StrategyKind {
    ...
    fn name(&self) -> &'static str {
        match self {
            ...
            Self::LinearAlphaScoreRotated => "LinearAlpha(score_rotated)",
...
async fn main() -> Result<()> {
    println!("=== CROSS-SECTIONAL LINEAR MODEL PORTFOLIO MAPPING AUDIT ===\n");
    println!("Goal: pressure-test whether portfolio-level edge still needs the score on the right symbol");
    ...
    println!(
        "Fees: {:.1}% taker each side | capped book: top-{} strengths",
        TAKER_FEE * 100.0,
        POSITION_CAP
    );
    println!(
        "Model train window: {} bars | shuffled_y rotate: {} | score rotate: {}\n",
        TRAIN_BARS, SHUFFLE_ROTATE_BY, SCORE_ROTATE_BY
    );
    ...
    let chart_paths = write_curve_exports_and_charts(&summaries)?;
    println!("\nSnapshots written:");
    println!("- {}", SNAPSHOT_LATEST_MD);
    println!("- {}", SNAPSHOT_LATEST_CSV);
    println!("- {}", archive_md);
    println!("- {}", archive_csv);
    println!("\nCurve exports / charts:");
    for path in chart_paths {
        println!("- {}", path);
    }

    println!("\nInterpretation:");
    println!("- If score_rotated compresses much harder than shuffled_y under the capped-book lens, portfolio-level trust improves.");
    println!("- If score_rotated stays too alive even here, the remaining edge is still compatible with generic ranking / book construction.");
    println!("- This is still trust work, not promotion.");
...
fn generate_all_signals(
    ...
    out.insert(
        ...
        generate_linear_alpha_outputs(data_cache, feature_set, symbols, false, false)?,
    );
    out.insert(
        StrategyKind::LinearAlphaShuffled,
        generate_linear_alpha_outputs(data_cache, feature_set, symbols, true, false)?,
    );
    out.insert(
        StrategyKind::LinearAlphaScoreRotated,
        generate_linear_alpha_outputs(data_cache, feature_set, symbols, false, true)?,
    );
...
fn generate_linear_alpha_outputs(
    ...
    for i in WARMUP_BARS.max(TRAIN_BARS)..(n.saturating_sub(HOLD_BARS + 1)) {
        ...
        if rotate_scores && scores.len() > 1 {
            let rotated_values: Vec<f64> = scores.iter().map(|(_, s)| *s).collect();
            let len = rotated_values.len();
            for (idx, (_, score)) in scores.iter_mut().enumerate() {
                *score = rotated_values[(idx + SCORE_ROTATE_BY) % len];
            }
        }
...
fn simulate_portfolio(plans: &[SymbolPlan], steps: usize) -> PortfolioStats {
    ...
    let drawdown_curve = calc_drawdown_curve(&equity_curve);
    PortfolioStats {
...
fn write_snapshot(summaries: &[UniverseSummary]) -> Result<(String, String)> {
    ...
    let archive_md = format!(
        "{}/cross_sectional_linear_model_portfolio_mapping_{}.md",
        SNAPSHOT_DIR, timestamp
    );
    let archive_csv = format!(
        "{}/cross_sectional_linear_model_portfolio_mapping_{}.csv",
        SNAPSHOT_DIR, timestamp
    );
...
fn render_markdown(summaries: &[UniverseSummary]) -> String {
    ...
    out.push_str("# Cross-Sectional Linear Model Portfolio Mapping Audit\n\n");
...
fn calc_drawdown_curve(equity_curve: &[f64]) -> Vec<f64> {
    let mut peak = equity_curve.first().copied().unwrap_or(1.0);
...
fn write_curve_exports_and_charts(summaries: &[UniverseSummary]) -> Result<Vec<String>> {
    fs::create_dir_all("charts")?;
    let mut paths = Vec::new();
    for summary in summaries {
        let slug = summary.label.to_lowercase();
        let csv_path = format!(
            "snapshots/cross_sectional_linear_model_portfolio_mapping_{}_curves.csv",
            slug
        );
        let equity_png = format!(
            "charts/cross_sectional_linear_model_portfolio_mapping_{}_equity.png",
            slug
        );
        let dd_png = format!(
            "charts/cross_sectional_linear_model_portfolio_mapping_{}_drawdown.png",
            slug
        );
        write_curve_csv(summary, &csv_path)?;
        draw_curve_chart(summary, false, &equity_png)?;
        draw_curve_chart(summary, true, &dd_png)?;
        paths.push(csv_path);
        paths.push(equity_png);
        paths.push(dd_png);
    }
    Ok(paths)
}

fn write_curve_csv(summary: &UniverseSummary, path: &str) -> Result<()> {
    let mut out = String::from("universe,strategy,series,index,value\n");
    for row in &summary.rows {
        for (i, v) in row.stats.equity_curve.iter().enumerate() {
            out.push_str(&format!(
                "{},{},equity,{},{}\n",
                summary.label,
                row.strategy.name(),
        ...
        for (i, v) in row.stats.drawdown_curve.iter().enumerate() {
            out.push_str(&format!(
                "{},{},drawdown,{},{}\n",
                summary.label,
                row.strategy.name(),
...
fn draw_curve_chart(summary: &UniverseSummary, drawdown: bool, path: &str) -> Result<()> {
    let root = BitMapBackend::new(path, (1500, 900)).into_drawing_area();
    root.fill(&WHITE)?;
    let title = if drawdown {
        format!("Drawdown curves — {}", summary.label)
    } else {
        format!("Equity curves (log scale) — {}", summary.label)
    };
    let max_len = summary
        .rows
        .iter()
        .map(|r| {
            if drawdown {
                r.stats.drawdown_curve.len()
            } else {
                r.stats.equity_curve.len()
            }
        })
        .max()
        .unwrap_or(1);
    let max_y = if drawdown {
        summary
            .rows
            .iter()
            .flat_map(|r| r.stats.drawdown_curve.iter().copied())
        ...
        summary
            .rows
            .iter()
            .flat_map(|r| r.stats.equity_curve.iter().copied())
    ...
    if drawdown {
        let mut chart = ChartBuilder::on(&root)
            ...
            .build_cartesian_2d(0..max_len, 0.0..max_y)?;
        chart
            .configure_mesh()
            .x_desc("Bar index")
            .y_desc("Drawdown %")
            .draw()?;
        let palette = [RED, BLUE, GREEN, BLACK, MAGENTA, CYAN];
        for (idx, row) in summary.rows.iter().enumerate() {
            let color = palette[idx % palette.len()];
            let series: Vec<(usize, f64)> = row
                .stats
                .drawdown_curve
                .iter()
                .enumerate()
        ...
        let mut chart = ChartBuilder::on(&root)
            ...
            .build_cartesian_2d(0..max_len, (min_y..max_y).log_scale())?;
        chart
            .configure_mesh()
            .x_desc("Bar index")
