//! T65: Exact Live-Bot Source-of-Truth Daily Equity Harness
//!
//! Purpose: answer "what would `src/live/bot.rs`, as coded today, have done?"
//!
...
//!
//! Outputs:
//! - snapshots/live_bot_exact_equity.csv
//! - snapshots/live_bot_exact_trades.csv
//! - snapshots/live_bot_exact_equity.md
...
fn mark_to_market_equity(realized_equity: f64, positions: &HashMap<String, PositionState>, data: &HashMap<String, SymData>, idx: usize, fee: f64) -> f64 {
...
async fn main() -> Result<()> {
    println!("=== T65: EXACT LIVE-BOT SOURCE-OF-TRUTH EQUITY ===");
    ...
    println!("Entry candidates: {}, ATR gate skips: {}, hedged entries: {}, open liquidated at end: {}", entry_candidates, atr_gate_skips, hedge_entries, open_at_end);
    ...
    let top5_log: f64 = top.iter().take(5).map(|t| t.equity_mult.max(1e-12).ln()).sum();
    let top10_log: f64 = top.iter().take(10).map(|t| t.equity_mult.max(1e-12).ln()).sum();
    let equity_without_top5 = if top5_log.is_finite() { (total_log - top5_log).exp() } else { 0.0 };
    ...
    File::create("snapshots/live_bot_exact_equity.csv")?.write_all(equity_csv.as_bytes())?;
    ...
    for t in &trades {
        trade_csv.push_str(&format!(
            ...
            t.entry_price, t.exit_price, t.size, t.pct_ret, t.equity_mult, t.exit_reason, t.hedge_active
        ));
    }
    File::create("snapshots/live_bot_exact_trades.csv")?.write_all(trade_csv.as_bytes())?;

    // Markdown report.
    let mut md = File::create("snapshots/live_bot_exact_equity.md")?;
    writeln!(md, "# T65: Exact Live-Bot Source-of-Truth Equity\n")?;
    writeln!(md, "Generated: {}\n", Utc::now().format("%Y-%m-%d %H:%M UTC"))?;
    writeln!(md, "## Scope\n")?;
    writeln!(md, "This harness replays the current `src/live/bot.rs` daily event logic over common timestamp-aligned Base5 symbols in configured symbol order. It is a source-of-truth audit harness, not a new hyperopt.\n")?;
    ...
    writeln!(md, "| Entry candidates before ATR gate | {} |", entry_candidates)?;
    writeln!(md, "| ATR gate skips | {} |", atr_gate_skips)?;
    writeln!(md, "| Hedged entries | {} |", hedge_entries)?;
    writeln!(md, "| Open positions liquidated at end | {} |\n", open_at_end)?;
    ...
    writeln!(md, "| Metric | Value |")?;
    writeln!(md, "|---|---:|")?;
    writeln!(md, "| Equity without top 5 log contributors | {:.2}x |", equity_without_top5)?;
    writeln!(md, "| Equity without top 10 log contributors | {:.2}x |", equity_without_top10)?;
    writeln!(md, "| Top 5 share of log return | {:.1}% |", if total_log.abs() > 1e-12 { top5_log / total_log * 100.0 } else { 0.0 })?;
    writeln!(md, "| Top 10 share of log return | {:.1}% |\n", if total_log.abs() > 1e-12 { top10_log / total_log * 100.0 } else { 0.0 })?;

    writeln!(md, "### Top 10 Trades\n")?;
    writeln!(md, "| Rank | Symbol | Entry | Exit | Held | Size | Trade Ret | Equity Mult | Reason | Hedge |")?;
    writeln!(md, "|---:|---|---|---|---:|---:|---:|---:|---|---|")?;
    for (rank, t) in top.iter().take(10).enumerate() {
        writeln!(md, "| {} | {} | {} | {} | {} | {:.3} | {:.1}% | {:.4} | {} | {} |",
            rank + 1, t.symbol, t.entry_date, t.exit_date, t.bars_held, t.size,
            t.pct_ret * 100.0, t.equity_mult, t.exit_reason, t.hedge_active)?;
    }
    writeln!(md, "\n## Files\n")?;
    writeln!(md, "- `snapshots/live_bot_exact_equity.csv` — daily mark-to-market account equity")?;
    writeln!(md, "- `snapshots/live_bot_exact_trades.csv` — trade ledger")?;

    println!("CSV: snapshots/live_bot_exact_equity.csv");
    println!("Trades: snapshots/live_bot_exact_trades.csv");
    println!("Report: snapshots/live_bot_exact_equity.md");
