...
impl StrategyKind {
    ...
    fn description(&self) -> &'static str {
        match self {
            ...
            Self::AdMomentum => "volume-pressure momentum baseline",
            Self::AdMomentumNoDOGE => "A/D with DOGE explicitly removed",
...
async fn main() -> Result<()> {
    println!("=== ENSEMBLE FAMILY BENCHMARK + HARSH UNIVERSE STRESS ===\n");
...
fn evaluate_strategy(
    ...
    for &symbol in symbols {
        ...
        let result = run_strategy(df, symbol, strategy, btc_sma_200)?;
...
fn evaluate_strategy_on_windows(
    ...
    for &symbol in symbols {
        let df = data_cache.get(symbol).context("missing symbol data")?;
        let result = run_strategy_on_windows(df, symbol, strategy, windows, btc_sma_200)?;
        aggregate.add_assign(&result);
    }
    Ok(aggregate)
}
...
fn run_strategy(
    ...
    let signal = signal_for_strategy(df, strategy, btc_sma_200, Some(symbol))?;
...
fn signal_for_strategy(
    ...
    let out = match strategy {
        ...
        StrategyKind::AdMomentumNoDOGE => {
            if symbol.map(|s| s.contains("DOGE")).unwrap_or(false) {
                vec![0i32; df.height()]
            } else {
                ad_momentum
            }
