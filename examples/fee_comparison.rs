...
async fn main() -> Result<()> {
    ...
    for symbol in SYMBOLS {
        for interval in INTERVALS {
            ...
            for (name, make_strat) in &strategy_fns {
                ...
                if r_taker.total_trades < MIN_TRADES {
                    continue;
                }
    ...
    for r in &rows {
        ...
        println!(
            "{:<22} {:<10} {:<6} {:>7} {:>9.1}% {} {:>+7.1}%",
            r.strategy, r.symbol, r.interval, r.trades, r.taker_ret, maker_str, edge
        );
    }
    ...
    println!(
        "  Taker round trip cost:  ${:.2}  (0.1%)",
        CAPITAL * TAKER_FEE * 2.0
    );
    println!(
        "  Maker round trip cost:  ${:.2}  (0.0%)",
        CAPITAL * MAKER_FEE * 2.0
    );
    println!(
        "  Savings per trade:      ${:.2}",
        CAPITAL * (TAKER_FEE - MAKER_FEE) * 2.0
    );
    println!(
        "  Savings on {:.0} trades:   ${:.2}",
        avg_trades,
    ...
    println!(
        ...
        avg_trades * (TAKER_FEE - MAKER_FEE) * 2.0 * 100.0,
        avg_trades
    );
