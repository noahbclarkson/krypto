...
const TRAILING_STOP: f64 = 0.15; // 15 %
...
async fn main() -> Result<()> {
    ...
    println!(
        "{}",
        "  KRYPTO — Full Strategy Backtest".bright_cyan().bold()
    );
    ...
    println!(
        "{}",
        "Phase 1 — Fetching data (cached after first run)".bright_green()
    );
    ...
    let backtester = Backtester::with_defaults(CAPITAL).with_position_sizing(PositionSizing::Full);
    ...
    println!(
        "{}",
        "  ALL RESULTS — sorted by composite score (return/DD × PF × win-rate)"
            .bright_cyan()
            .bold()
    );
    println!("{}", "━".repeat(100).bright_cyan());
    ...
    for r in &results {
        ...
        let mark = if r.total_trades < MIN_TRADES {
            "*".yellow()
        } else {
            " ".normal()
        };
    ...
    println!(
        "{} * fewer than {MIN_TRADES} trades — statistically unreliable",
        " ".normal()
    );
    ...
    println!(
        "{}",
        "  Strategy Average Returns (across all symbols/intervals)"
            .bright_cyan()
            .bold()
    );
    println!("{}", "━".repeat(72).bright_cyan());
    ...
    for r in &results {
        by_strat
            ...
            .or_default()
            .push(r.total_return_pct);
    }
    let mut strat_avgs: Vec<(String, f64, usize)> = by_strat
        .iter()
    ...
    for (i, (name, avg, count)) in strat_avgs.iter().enumerate() {
        ...
        println!(
            "  {:>2}. {:<26} avg return {avg_str}  ({count} runs)",
            i + 1,
            name
        );
    }
    ...
    println!(
        "{}",
        "  Symbol Average Returns (across all strategies/intervals)"
            .bright_cyan()
            .bold()
    );
    println!("{}", "━".repeat(72).bright_cyan());
    ...
    let mut sym_avgs: Vec<(String, f64)> = by_sym
        .iter()
    ...
    println!(
        "{}",
        "  Interval Average Returns (across all strategies/symbols)"
            .bright_cyan()
            .bold()
    );
    println!("{}", "━".repeat(72).bright_cyan());
    ...
    let mut int_avgs: Vec<(String, f64)> = by_int
        .iter()
        .map(|(k, v)| (k.clone(), v.iter().sum::<f64>() / v.len() as f64))
        .collect();
    ...
    let profitable = results
        .iter()
        .filter(|r| r.total_return_pct > 0.0 && r.total_trades >= MIN_TRADES)
        .count();
    let sig = results
        .iter()
        .filter(|r| r.total_trades >= MIN_TRADES)
        .count();
    let best = results.iter().find(|r| r.total_trades >= MIN_TRADES);
    let avg_ret =
        results.iter().map(|r| r.total_return_pct).sum::<f64>() / results.len().max(1) as f64;
