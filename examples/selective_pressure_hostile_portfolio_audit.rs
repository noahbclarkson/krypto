//! Selective hostile-state pressure audit: can pressure earn allocator attention only when the
//! slow-pressure map is actually bearish?
//!
//! Purpose:
//! - avoid another standalone leaderboard or parameter loop after pressure survived only as a
//!   secondary family / attribution object
//! - test the more honest next object: only add or swap toward pressure when the observable
//!   slow-pressure state is actually hostile (`PressureBearish`)
//! - answer a trust question, not a promotion question: does selective hostile-state pressure add
//!   anything, or was the earlier pressure result mostly a contextual diagnostic?
//!
...
//! - slow-pressure state is computed from cross-sectional SlowPressure(7,14,28) breadth
//!
//! Honesty note:
//! - both weekly pressure and slow-pressure state are still OHLCV-derived proxies, not true
//!   taker-flow / liquidation / order-book state
//! - this is a selective-use audit, not a promotion result
...
const SNAPSHOT_LATEST_MD: &str = "snapshots/selective_pressure_hostile_portfolio_audit_latest.md";
const SNAPSHOT_LATEST_CSV: &str = "snapshots/selective_pressure_hostile_portfolio_audit_latest.csv";
...
impl StrategyKind {
    ...
    fn name(&self) -> &'static str {
        match self {
            ...
            Self::WeeklyPressureSmall => "WeeklyPressure+Small",
        }
    }
}
...
enum PortfolioKind {
    ...
    DdBudgetAllFour,
    BearPressureAdd,
    BearPressureSwapSmall,
    BearPressureSwapMacd,
}
...
impl PortfolioKind {
    fn all() -> &'static [PortfolioKind] {
        &[
            ...
            Self::DdBudgetAllFour,
            Self::BearPressureAdd,
            Self::BearPressureSwapSmall,
            Self::BearPressureSwapMacd,
        ]
    }
    ...
    fn name(&self) -> &'static str {
        match self {
            ...
            Self::DdBudgetAllFour => "DDBudget(A/D,MACD,Small,Pressure)",
            Self::BearPressureAdd => "BearPressureAdd(A/D,MACD,Small,+P)",
            Self::BearPressureSwapSmall => "BearPressureSwapSmall(A/D,MACD,P)",
            Self::BearPressureSwapMacd => "BearPressureSwapMACD(A/D,Small,P)",
        }
    }
}
...
async fn main() -> Result<()> {
    println!("=== SELECTIVE PRESSURE HOSTILE-PORTFOLIO AUDIT ===\n");
    println!("Question: does pressure help only when the slow-pressure state is actually hostile, or was it mainly an attribution object?");
    println!("Allocators: frozen three-sleeve book vs always-on fourth sleeve vs bearish-state selective pressure variants.");
    println!(
    ...
    println!("Honesty note: both pressure sleeve and hostile-state map are still OHLCV-derived flow proxies, not true taker-flow.\n");
    ...
    for &(label, universe_symbols) in UNIVERSES {
        ...
        let pressure_signal_map =
            generate_weekly_pressure_small_signal_strengths(&universe, &small_signal_map)?;
        let slow_pressure_signal_map =
            generate_slow_pressure_signal_strengths(&universe, (7, 14, 28), (0.45, 0.35, 0.20))?;
        let pressure_state = classify_pressure_state(&slow_pressure_signal_map)?;
        ...
        for &portfolio in PortfolioKind::all() {
            let stats =
                simulate_family_portfolio(&sleeves, universe.steps, &pressure_state, portfolio);
...
fn generate_weekly_pressure_small_signal_strengths(
    universe: &UniverseData,
    ...
    for (symbol, df) in &universe.data {
        ...
        let flow_10 = rolling_sum_vec(&flow, 10);
        let flow_21 = rolling_sum_vec(&flow, 21);
        let dv_21 = rolling_dollar_volume(close, volume, 21);
        let dv_63 = rolling_dollar_volume(close, volume, 63);
        let (small_signals, small_strengths) = small_signal_map
            .get(symbol)
            .context("missing small map for weekly pressure sleeve")?;

        let mut scores = vec![0.0; n];
        for i in 63..n {
            let pressure = 0.50 * (flow_5[i] / dv_21[i].max(1.0))
                + 0.30 * (flow_10[i] / dv_21[i].max(1.0))
                + 0.20 * (flow_21[i] / dv_63[i].max(1.0));
            let small_dir = small_signals.get(i).copied().unwrap_or(0) as f64;
            let small_strength = small_strengths.get(i).copied().unwrap_or(0.0);
            scores[i] = if small_dir.abs() > 0.0 && pressure.signum() == small_dir.signum() {
                pressure.abs() * (1.0 + 0.25 * small_strength) * small_dir.signum()
            } else {
                0.0
            };
        }
...
fn generate_slow_pressure_signal_strengths(
    universe: &UniverseData,
    ...
    for (symbol, df) in &universe.data {
        ...
        let dv_a = rolling_dollar_volume(close, volume, windows.0.max(7));
        let dv_b = rolling_dollar_volume(close, volume, windows.1.max(14));
        let dv_c = rolling_dollar_volume(close, volume, windows.2.max(21));

        let mut scores = vec![0.0; n];
        for i in windows.2..n {
            scores[i] = weights.0 * (flow_a[i] / dv_a[i].max(1.0))
                + weights.1 * (flow_b[i] / dv_b[i].max(1.0))
                + weights.2 * (flow_c[i] / dv_c[i].max(1.0));
        }
        raw_scores.insert(symbol.clone(), scores);
    }
...
fn classify_pressure_state(
    slow_pressure_signal_map: &HashMap<String, (Vec<i32>, Vec<f64>)>,
) -> Result<Vec<PressureStateBucket>> {
    let n = slow_pressure_signal_map
        .values()
        .next()
        .map(|(signals, _)| signals.len())
        .unwrap_or(0);
    let symbols = slow_pressure_signal_map.keys().cloned().collect::<Vec<_>>();
    let mut out = vec![PressureStateBucket::Mixed; n];
    ...
    for i in 0..n {
        ...
        for symbol in &symbols {
            let signal = slow_pressure_signal_map
                .get(symbol)
                .and_then(|(signals, _)| signals.get(i))
                .copied()
                .unwrap_or(0);
            if signal > 0 {
                pos += 1;
        ...
        let pos_share = pos as f64 / total;
        let neg_share = neg as f64 / total;
        let active_share = active as f64 / total;

        out[i] = if active_share < 0.25 {
            PressureStateBucket::Quiet
        } else if neg_share >= 0.50 && neg > pos {
            PressureStateBucket::Bearish
        } else if pos_share >= 0.50 && pos > neg {
...
fn build_symbol_plans(
    ...
    strategy: StrategyKind,
    small_signal_map: &HashMap<String, (Vec<i32>, Vec<f64>)>,
    pressure_signal_map: &HashMap<String, (Vec<i32>, Vec<f64>)>,
) -> Result<Vec<SymbolPlan>> {
    universe
        .data
        .iter()
        .map(|(symbol, df)| {
            build_symbol_plan(df, symbol, strategy, small_signal_map, pressure_signal_map)
        })
        .collect()
}
...
fn build_symbol_plan(
    ...
    let (signals, strengths) = match strategy {
        ...
        StrategyKind::WeeklyPressureSmall => pressure_signal_map
            .get(symbol)
            .cloned()
            .context("missing weekly pressure sleeve signal map")?,
    };
    ...
    for day in 0..steps {
        ...
        for sleeve in sleeves {
            ...
            let state = pressure_state
                .get(day)
                .copied()
                .unwrap_or(PressureStateBucket::Mixed);
...
fn included_strategies(portfolio: PortfolioKind) -> &'static [StrategyKind] {
    match portfolio {
        ...
        PortfolioKind::DdBudgetAllFour => &[
            StrategyKind::AdMomentum,
            StrategyKind::MacdRegime,
            StrategyKind::SmallByDollarVol,
            StrategyKind::WeeklyPressureSmall,
        ],
        PortfolioKind::BearPressureAdd => &[
            StrategyKind::AdMomentum,
            StrategyKind::MacdRegime,
            StrategyKind::SmallByDollarVol,
            StrategyKind::WeeklyPressureSmall,
        ],
        PortfolioKind::BearPressureSwapSmall => &[
            StrategyKind::AdMomentum,
            StrategyKind::MacdRegime,
            StrategyKind::WeeklyPressureSmall,
        ],
        PortfolioKind::BearPressureSwapMacd => &[
            StrategyKind::AdMomentum,
            StrategyKind::SmallByDollarVol,
            StrategyKind::WeeklyPressureSmall,
        ],
...
fn sleeve_weight(
    portfolio: PortfolioKind,
    strategy: StrategyKind,
    state: PressureStateBucket,
    dd_pct: f64,
    recovery_ratio: f64,
    _global_dd: f64,
    _global_recovery: f64,
) -> f64 {
    let live = ddhard_exposure(dd_pct, recovery_ratio);
    match portfolio {
        PortfolioKind::DdBudgetAllThree => match strategy {
            StrategyKind::AdMomentum
            | StrategyKind::MacdRegime
            | StrategyKind::SmallByDollarVol => (1.0 / 3.0) * live,
            _ => 0.0,
        },
        PortfolioKind::DdBudgetAllFour => match strategy {
            StrategyKind::AdMomentum
            | StrategyKind::MacdRegime
            | StrategyKind::SmallByDollarVol
            | StrategyKind::WeeklyPressureSmall => 0.25 * live,
        },
        PortfolioKind::BearPressureAdd => match state {
            PressureStateBucket::Bearish => match strategy {
                StrategyKind::AdMomentum
                | StrategyKind::MacdRegime
                | StrategyKind::SmallByDollarVol
                | StrategyKind::WeeklyPressureSmall => 0.25 * live,
        ...
        PortfolioKind::BearPressureSwapSmall => match state {
            PressureStateBucket::Bearish => match strategy {
                StrategyKind::AdMomentum
                | StrategyKind::MacdRegime
                | StrategyKind::WeeklyPressureSmall => (1.0 / 3.0) * live,
                _ => 0.0,
            },
            _ => match strategy {
                StrategyKind::AdMomentum
        ...
        PortfolioKind::BearPressureSwapMacd => match state {
            PressureStateBucket::Bearish => match strategy {
                StrategyKind::AdMomentum
                | StrategyKind::SmallByDollarVol
                | StrategyKind::WeeklyPressureSmall => (1.0 / 3.0) * live,
...
fn write_snapshot(universe_summaries: &[UniverseSummary]) -> Result<(String, String)> {
    ...
    let archive_md = format!(
        "{}/selective_pressure_hostile_portfolio_audit_{}.md",
        SNAPSHOT_DIR, ts
    );
    let archive_csv = format!(
        "{}/selective_pressure_hostile_portfolio_audit_{}.csv",
        SNAPSHOT_DIR, ts
    );

    let mut md = String::from("# Selective Pressure Hostile-Portfolio Audit\n\n");
