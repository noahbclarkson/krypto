//! Cluster-residual reversion breadth benchmark.
//!
//! Goal:
//! - test an under-served Track C family built around stable peer clusters
//! - ask whether symbols mean-revert after overshooting their own calibrated
//!   peer-cluster baseline
//! - keep the same fair daily harness used elsewhere so results stay comparable
//!
//! Construction:
//! - calibrate each symbol's top stable peers from an early fixed window using
//!   lagged-return correlation
//! - compute a 21-bar relative return residual = symbol return minus peer-cluster mean return
//! - trade reversion: long the biggest negative residuals and short the biggest positive residuals
//!
...
impl StrategyKind {
    fn name(&self) -> &'static str {
        match self {
            Self::ClusterResidualReversion => "ClusterResidualReversion",
...
async fn main() -> Result<()> {
    println!("=== CLUSTER RESIDUAL REVERSION BREADTH BENCHMARK ===\n");
    ...
    println!("Stable peers kept per symbol: {}", PEER_COUNT);
    println!(
    ...
    println!("\nInterpretation:");
    println!("- ClusterResidualReversion asks whether symbols mean-revert after overshooting their own stable peer cluster.");
    println!("- If it fails, that is still useful: it means simple peer-relative dispersion is not a clean daily edge.");
...
fn generate_all_signals(
    ...
    out.insert(
        StrategyKind::ClusterResidualReversion,
        generate_cluster_residual_signals(data_cache, symbols)?,
    );
...
fn generate_cluster_residual_signals(
    data_cache: &HashMap<String, DataFrame>,
    symbols: &[&str],
    ...
    let peer_map = select_cluster_peers(symbols, &ret_map);
    for &symbol in symbols {
...
fn select_cluster_peers(
    symbols: &[&str],
    ret_map: &HashMap<String, Vec<f64>>,
) -> HashMap<String, Vec<String>> {
    let mut out = HashMap::new();
    for &symbol in symbols {
        let Some(base) = ret_map.get(symbol) else {
            continue;
        };
        let end = CALIBRATION_BARS.min(base.len());
        let mut peers = Vec::<(String, f64)>::new();
        for &other in symbols {
            if other == symbol {
                continue;
            }
            if let Some(other_ret) = ret_map.get(other) {
                let corr = correlation(&base[..end], &other_ret[..end]);
                if corr.is_finite() && corr > 0.10 {
                    peers.push((other.to_string(), corr));
                }
            }
        }
...
fn write_snapshot_files(
    ...
    fs::write(
        format!("snapshots/cluster_residual_reversion_benchmark_{}.md", ts),
        &md,
    )?;
    fs::write(
        format!("snapshots/cluster_residual_reversion_benchmark_{}.csv", ts),
        &csv,
    )?;
    Ok(())
}
