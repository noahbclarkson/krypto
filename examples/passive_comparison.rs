...
use krypto::{
    ...
    backtest::passive::{PassiveConfig, PassiveExecutor, TickSize},
...
async fn main() -> Result<()> {
    ...
    println!(
        "  1m bars in first 1h bar: {} (expected: 60)",
        bars_in_first
    );
    ...
    let (_, stats_anchored) = executor_anchored
        .simulate(&df_high, &df_low, &signals)
        .await?;

    println!("  Fill rate: {:.1}%", stats_anchored.fill_rate * 100.0);
    println!(
        "  Avg price improvement: {:.2} ticks",
        stats_anchored.avg_price_improvement_ticks
    );
    ...
    println!(
        "  Avg price improvement: {:.2} ticks",
        stats_walk.avg_price_improvement_ticks
    );
    ...
    let (_, stats_threshold) = executor_threshold
        .simulate(&df_high, &df_low, &signals)
        .await?;
    ...
    println!(
        "  Avg price improvement: {:.2} ticks",
        stats_threshold.avg_price_improvement_ticks
    );
    println!(
        "  Avg bars to fill: {:.1}",
        stats_threshold.avg_bars_to_fill
    );

    println!("\n{}", "━".repeat(72).bright_cyan());
    println!(
        "{}",
        "THE REAL EDGE: 0% MAKER FEES ON FDUSD"
            .bright_green()
            .bold()
    );
    println!("{}", "━".repeat(72).bright_cyan());
    ...
    println!(
        "\n  {} trades/year @ ${} avg position",
        trades_per_year as u32, avg_trade_size as u32
    );
    println!("\n  {:20} {:>12} {:>12}", "Fee Type", "Rate", "Annual Cost");
    println!("  {}", "-".repeat(44));
    println!(
        "  {:20} {:>12} ${:>10.2}",
        "Taker (market)", "0.050%", taker_cost
    );
    println!(
        "  {:20} {:>12} ${:>10.2}",
        "Maker (FDUSD)", "0.000%", maker_cost_fdusd
    );
    println!(
        "  {:20} {:>12} ${:>10.2}",
        "Maker (non-FDUSD)", "0.020%", maker_cost_other
    );
    ...
    println!(
        ...
        "FDUSD vs Taker",
        "",
        taker_cost - maker_cost_fdusd
    );
    println!(
        "  {:20} {:>12} ${:>10.2} / year",
        "FDUSD vs non-FDUSD",
        "",
        maker_cost_other - maker_cost_fdusd
    );
    ...
    println!(
        "  Savings in bps: {:.1} bps/trade",
        savings_pct * 100.0 / trades_per_year
    );
