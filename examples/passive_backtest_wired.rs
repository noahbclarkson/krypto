...
async fn main() -> Result<()> {
    ...
    println!(
        "{}",
        "  PASSIVE BACKTEST — Wired into Engine"
            .bright_cyan()
            .bold()
    );
    ...
    print!(
        "Fetching {} {} ({} bars)... ",
        SYMBOL, INTERVAL_HIGH, CANDLES_HIGH
    );
    let df_high = FeatureEngine::add_technicals(
        &loader
            .fetch_data(SYMBOL, INTERVAL_HIGH, CANDLES_HIGH)
            .await?,
        None,
    )?;
    println!("{}", "✓".green());
    ...
    print!(
        "Fetching {} {} ({} bars)... ",
        SYMBOL, INTERVAL_LOW, CANDLES_LOW
    );
    ...
    println!("\n{}", "Notes:".bright_yellow());
    println!(
        "  • Fill% = % of signals that got a passive fill within {} 1m bars",
        passive_cfg.max_wait_bars
    );
    println!("  • AvgBars = average 1m bars until fill");
    println!("  • Edge = passive return - taker return (fee savings + price improvement)");
    println!(
        "  • Passive uses anchor_to_signal=true: limit set once at signal close - {} ticks\n",
