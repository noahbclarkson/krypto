...
fn load_sample_data() -> Vec<Bar> {
    ...
    let content = std::fs::read_to_string(path).expect("Failed to read sample data file");

    let json_bars: Vec<JsonBar> = serde_json::from_str(&content).expect("Failed to parse JSON");
...
fn print_summary(summary: &BotSummary) {
    ...
    println!(
        ...
        "", summary.directional_accuracy, summary.total_fees,
    );
}
...
fn main() {
    ...
    if passing.is_empty() {
        ...
        println!(
            "✅ {} strateg(y/ies) meet the {:.0}% target:",
            passing.len(),
            target
        );
        for s in passing {
    ...
    println!("\n=== BEST STRATEGY ===");
    println!(
        "{} with {:.2}% directional accuracy",
        best.strategy, best.directional_accuracy
    );
}
