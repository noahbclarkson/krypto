//! Residualized ticker-interaction / cross-impact breadth benchmark.
//!
//! Goal:
//! - test whether the earlier old-guard cross-impact positive is mostly broad crypto-market mode
//!   or whether some interaction signal survives after removing a simple BTC factor
//! - keep the same fair daily assumptions as the other breadth scans
//! - treat this as a breadth falsification / narrowing pass, not a deployment decision
...
impl StrategyKind {
    fn name(&self) -> &'static str {
        match self {
            ...
            Self::CrossImpactResidualConfirm => "Residual BTC + MACD conf",
    ...
    fn all() -> &'static [StrategyKind] {
        &[
            ...
            Self::CrossImpactResidualConfirm,
...
async fn main() -> Result<()> {
    println!("=== CROSS-IMPACT RESIDUAL BENCHMARK ===\n");
    println!(
    ...
    println!(
        "Goal: ask whether cross-impact survives after removing a simple BTC market component\n"
    );
    ...
    println!("\nInterpretation:");
    println!("- If residualized cross-impact collapses versus the blunt all-peer rule, the earlier positive was mostly broad crypto-market mode / basket confirmation.");
    println!("- If it survives on old-guard baskets, that is stronger evidence for interaction structure beyond simple BTC beta.");
...
fn generate_signals(
    ...
    match strategy {
        StrategyKind::CrossImpactSlow => Ok(generate_cross_impact_signals(inputs, false, None)),
        StrategyKind::CrossImpactResidualBtc => {
            Ok(generate_cross_impact_signals(inputs, true, None))
        }
        StrategyKind::CrossImpactResidualConfirm => {
            let macd = generate_macd_regime_signals(df)?;
            Ok(generate_cross_impact_signals(inputs, true, Some(&macd)))
        }
...
fn generate_cross_impact_signals(
    ...
    for i in PEER_LOOKBACK.max(BTC_BETA_LOOKBACK)..n {
        ...
        let raw_signal = if peer_avg >= threshold && same_sign >= 2 {
            1
        ...
        if let Some(confirm_signals) = confirm {
            if confirm_signals.get(i).copied().unwrap_or(0) != raw_signal {
                continue;
            }
        }
