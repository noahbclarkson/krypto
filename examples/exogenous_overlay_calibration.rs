...
const SNAPSHOT_LATEST_MD: &str = "snapshots/exogenous_overlay_calibration_latest.md";
const SNAPSHOT_LATEST_CSV: &str = "snapshots/exogenous_overlay_calibration_latest.csv";
...
impl OverlayKind {
    ...
    fn name(&self) -> &'static str {
        match self {
            ...
            Self::StressLean75 => "StressLean75(1.0 stress / 0.75 else)",
            Self::StressTilt50 => "StressTilt50(1.0 stress / 0.50 else)",
            Self::StressLean25 => "StressLean25(1.0 stress / 0.25 else)",
    ...
    fn exposure(&self, state: MacroStateBucket) -> f64 {
        match self {
            Self::Baseline => 1.0,
            Self::StressLean75 => match state {
                MacroStateBucket::Stress => 1.0,
                MacroStateBucket::RiskOn | MacroStateBucket::Neutral => 0.75,
            },
            Self::StressTilt50 => match state {
                MacroStateBucket::Stress => 1.0,
                MacroStateBucket::RiskOn | MacroStateBucket::Neutral => 0.50,
            },
            Self::StressLean25 => match state {
                MacroStateBucket::Stress => 1.0,
                MacroStateBucket::RiskOn | MacroStateBucket::Neutral => 0.25,
            },
...
async fn main() -> Result<()> {
    println!("=== EXOGENOUS OVERLAY CALIBRATION ===\n");
    println!(
        "Goal: calibrate gentle lagged macro-state exposure overlays under the capped-book lens"
    );
    ...
    println!("\nInterpretation:");
    println!("- Gentle stress tilts matter only if they improve Sharpe / DD enough before return decay becomes too large.");
    println!("- If only very harsh down-scaling helps, the macro signal is probably too blunt for practical sizing.");
    println!("- This is a portfolio-risk calibration, not a new strategy promotion result.");
...
fn write_snapshot(universes: &[UniverseSummary]) -> Result<(String, String)> {
    ...
    let archive_md = format!(
        "{}/exogenous_overlay_calibration_{}.md",
        SNAPSHOT_DIR, timestamp
    );
    let archive_csv = format!(
        "{}/exogenous_overlay_calibration_{}.csv",
        SNAPSHOT_DIR, timestamp
    );

    let mut markdown = String::new();
    markdown.push_str("# Exogenous Overlay Calibration\n\n");
