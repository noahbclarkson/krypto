...
use colored::*;
use krypto::{
    algo::strategies::{BollingerReversion, DynamicTrend, RsiMeanReversion},
...
async fn main() -> Result<()> {
    ...
    println!(
        "{}",
        "  PASSIVE EXECUTION — Entry Price Edge"
            .bright_cyan()
            .bold()
    );
    ...
    println!(
        "{}",
        "  Passive Execution Edge (per trade)".bright_cyan().bold()
    );
    println!("{}", "━".repeat(100).bright_cyan());
    println!(
        "  {:<18} {:<12} {:<5} {:>10} {:>10} {:>10} {:>8} {:>8}",
