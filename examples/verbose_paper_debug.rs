//! Debug why paper bot has so few trades.
...
async fn main() -> Result<()> {
    println!("\n{}", "━".repeat(80));
    println!("  DEBUG PAPER BOT (verbose, bars 100-160)");
    println!("{}", "━".repeat(80));
    ...
    // Create paper bot with verbose adapter
    let registry = StrategyRegistry::new();
    let strategy = registry.create("bollinger_reversion").unwrap();
    let adapter = SignalGeneratorAdapter::new(strategy, 100, ATR_MULT).with_verbose();
    let mut bot = PaperBot::new(Box::new(adapter), INITIAL_CAPITAL).with_fee(FEE);

    println!(
        "\n{:<6} | {:<10} | {:<12} | {:<10}",
        "Bar", "Close", "Position", "Equity"
    );
    println!("{}", "-".repeat(60));
    ...
    for (idx, bar) in bars.iter().enumerate() {
        ...
        if idx >= 100 && idx <= 160 {
            let pos_str = match position_after {
            ...
            if equity_before != equity_after || position_before != position_after {
                println!(
                    "{:<6} | {:>10.2} | {:<12} | {:>10.2} (CHANGED)",
                    idx, bar.close, pos_str, equity_after
                );
            } else {
                println!(
                    "{:<6} | {:>10.2} | {:<12} | {:>10.2}",
