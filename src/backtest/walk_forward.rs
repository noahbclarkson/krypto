...
impl WalkForwardResult {
    ...
    pub fn print_summary(&self) {
        ...
        println!(
            ...
            self.combined_total_return_pct, self.combined_sharpe, self.combined_max_drawdown
        );
...
fn aggregate_results(
    ...
    let avg_test_sharpe =
        windows.iter().map(|w| w.test.sharpe_ratio).sum::<f64>() / windows_total as f64;
    let avg_test_return_pct =
        windows.iter().map(|w| w.test.total_return_pct).sum::<f64>() / windows_total as f64;
