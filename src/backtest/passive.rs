                ...
                if let Some(sym_name) = sym_info.get("symbol").and_then(|s| s.as_str()) {
                    if let Some(filters) = sym_info.get("filters").and_then(|f| f.as_array()) {
                        for filter in filters {
                            if filter.get("filterType").and_then(|t| t.as_str())
                                == Some("PRICE_FILTER")
                            {
                                if let Some(tick_str) =
                                    filter.get("tickSize").and_then(|t| t.as_str())
                                {
...
mod tests {
    ...
    async fn test_high_fill_rate_long() {
        ...
        let (_fills, stats) = executor
            .simulate(&df_high, &df_low, &signals)
            .await
            .unwrap();
    ...
    #[test]
    fn test_lower_interval_for_signal_1h() {
        assert_eq!(lower_interval_for_signal("1h"), "5m");
    }

    #[test]
    fn test_lower_interval_for_signal_4h() {
        assert_eq!(lower_interval_for_signal("4h"), "15m");
    }

    #[test]
    fn test_lower_interval_for_signal_1d() {
        assert_eq!(lower_interval_for_signal("1d"), "30m");
    }

    #[test]
    fn test_lower_interval_for_signal_fallback() {
        assert_eq!(lower_interval_for_signal("unknown"), "5m");
    }

    #[test]
    fn test_bars_per_signal_1h() {
        // 1h → 5m → 60/5 = 12 bars
        assert_eq!(bars_per_signal("1h"), 12);
    }

    #[test]
    fn test_bars_per_signal_4h() {
        // 4h → 15m → 240/15 = 16 bars
        assert_eq!(bars_per_signal("4h"), 16);
    }

    #[test]
    fn test_bars_per_signal_1d() {
        // 1d → 30m → 1440/30 = 48 bars
        assert_eq!(bars_per_signal("1d"), 48);
    }

    #[test]
    fn test_tick_size_from_value() {
        let tick = TickSize::from_value(0.01);
        assert!((tick.value() - 0.01).abs() < 1e-10);
    }

    #[test]
    fn test_tick_size_fallback_btc() {
        let tick = TickSize::fallback("BTCUSDT");
        assert!((tick - 0.01).abs() < 1e-10);
    }

    #[test]
    fn test_tick_size_fallback_eth() {
        let tick = TickSize::fallback("ETHUSDT");
        assert!((tick - 0.001).abs() < 1e-10);
    }

    #[test]
    fn test_tick_size_fallback_xrp() {
        let tick = TickSize::fallback("XRPUSDT");
        assert!((tick - 0.00001).abs() < 1e-10);
    }

    #[test]
    fn test_passive_config_default() {
        let config = PassiveConfig::default();
        assert_eq!(config.ticks_below_open, 3);
        assert_eq!(config.max_wait_bars, 60);
        assert!((config.maker_fee).abs() < 1e-10);
        assert!(config.anchor_to_signal);
    }

    #[test]
    fn test_passive_executor_new() {
        let config = PassiveConfig::default();
        let executor = PassiveExecutor::new(config);
        assert_eq!(executor.config.ticks_below_open, 3);
    }

    #[test]
    fn test_passive_executor_with_defaults() {
        let executor = PassiveExecutor::with_defaults();
        assert_eq!(executor.config.ticks_below_open, 3);
    }

    #[test]
    fn test_interval_to_seconds_common() {
        ...
        assert_eq!(interval_to_seconds("4h"), Some(14400));
        assert_eq!(interval_to_seconds("1d"), Some(86400));
        assert_eq!(interval_to_seconds("1w"), Some(604800));
    }

    #[test]
    fn test_interval_to_seconds_unknown() {
        assert_eq!(interval_to_seconds("unknown"), None);
