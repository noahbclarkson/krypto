...
impl ExperimentRunner {
    ...
    pub fn run(&mut self) -> Result<ResultsSummary> {
        ...
        let summary =
            self.aggregate_results(&all_train_results, &all_test_results, &split_infos)?;
    ...
    fn run_split(
        ...
        // Slice data for train and test
        let train_df = data.slice(
            split.train_range.0 as i64,
            split.train_range.1 - split.train_range.0,
        );
        let test_df = data.slice(
            split.test_range.0 as i64,
            split.test_range.1 - split.test_range.0,
        );
    ...
    fn execute_backtest<S: SignalGenerator>(
        ...
        // Generate signals
        let train_signals = strategy.predict(train_df).with_context(|| {
            format!(
                "Failed to generate signals for strategy {} on train data",
                strategy.name()
            )
        })?;

        let test_signals = strategy.predict(test_df).with_context(|| {
            format!(
                "Failed to generate signals for strategy {} on test data",
                strategy.name()
            )
        })?;

        // Run backtests
        let train_result = if let Some(ref passive_cfg) = self.config.costs.passive_execution {
            // Use passive execution if configured
            let (res, _) = futures::executor::block_on(backtester.run_with_passive(
                train_df,
                train_df,
                &train_signals,
                trailing_stop,
            ...
            backtester
                .run(train_df, &train_signals, trailing_stop, take_profit)
                .with_context(|| "Train backtest failed")?
        };

        let test_result = if let Some(ref passive_cfg) = self.config.costs.passive_execution {
            // Use passive execution if configured
            let (res, _) = futures::executor::block_on(backtester.run_with_passive(
                test_df,
                test_df,
            ...
            backtester
                .run(test_df, &test_signals, trailing_stop, take_profit)
    ...
    fn aggregate_results(
        ...
        let average = BacktestMetrics {
            ...
            sortino_ratio: test_results.iter().map(|r| r.sortino_ratio).sum::<f64>()
                / test_results.len() as f64,
            calmar_ratio: test_results.iter().map(|r| r.calmar_ratio).sum::<f64>()
                / test_results.len() as f64,
            avg_trade_duration_bars: test_results
                .iter()
                .map(|r| r.avg_trade_duration_bars)
                .sum::<f64>()
                / test_results.len() as f64,
            max_consecutive_wins: test_results
                .iter()
                .map(|r| r.max_consecutive_wins)
                .max()
                .unwrap_or(0),
            max_consecutive_losses: test_results
                .iter()
                .map(|r| r.max_consecutive_losses)
                .max()
                .unwrap_or(0),
            avg_win_pct: test_results.iter().map(|r| r.avg_win_pct).sum::<f64>()
                / test_results.len() as f64,
            avg_loss_pct: test_results.iter().map(|r| r.avg_loss_pct).sum::<f64>()
                / test_results.len() as f64,
            largest_win_pct: test_results.iter().map(|r| r.largest_win_pct).sum::<f64>()
                / test_results.len() as f64,
            largest_loss_pct: test_results.iter().map(|r| r.largest_loss_pct).sum::<f64>()
                / test_results.len() as f64,
        };
...
mod tests {
    ...
    fn test_data_flow_integration() {
        ...
        let df_with_features =
            FeatureEngine::add_technicals(&raw_df, None).expect("Failed to compute features");

        // Verify features were added
        assert!(
            df_with_features.column("rsi").is_ok(),
            "RSI should be computed"
        );
        assert!(
            df_with_features.column("atr").is_ok(),
            "ATR should be computed"
        );
        assert!(
            df_with_features.column("macd").is_ok(),
        ...
        let result = backtester
            .run(&df_with_features, &signals, 0.05, 0.0)
    ...
    fn test_feature_validation_missing_columns() {
        ...
        assert!(
            ...
            "Error should mention missing 'open' column"
        );
    }
    ...
    fn test_data_split_slicing() {
        ...
        let train_df = df.slice(
            split.train_range.0 as i64,
            split.train_range.1 - split.train_range.0,
        );
        let test_df = df.slice(
            split.test_range.0 as i64,
            split.test_range.1 - split.test_range.0,
        );
        ...
        assert_eq!(
            train_close.get(0).unwrap(),
            0.0,
            "Train should start at index 0"
        );
        assert_eq!(
            train_close.get(59).unwrap(),
            59.0,
            "Train should end at index 59"
        );
    }
    ...
    fn test_run_backtest_on_loaded_data() {
        ...
        // Verify train result
        assert!(
            train_result.final_equity > 0.0,
            "Train equity should be positive"
        );
        assert!(
            train_result.equity_curve.len() == 70,
            "Train equity curve length should match"
        );
        assert!(
            train_result.sharpe_ratio.is_finite(),
            "Sharpe ratio should be finite"
        );

        // Verify test result
        assert!(
            test_result.final_equity > 0.0,
            "Test equity should be positive"
        );
        assert!(
            test_result.equity_curve.len() == 30,
            "Test equity curve length should match"
        );

        // Verify V3 extended metrics are populated
        assert!(
            train_result.sortino_ratio.is_finite(),
            "Sortino ratio should be finite"
        );
        assert!(
            train_result.calmar_ratio.is_finite(),
            "Calmar ratio should be finite"
        );
        assert!(
            train_result.avg_trade_duration_bars >= 0.0,
            "Avg duration should be non-negative"
        );

        println!(
            "Train result: {} trades, {:.2}% return, {:.2} Sharpe, {:.2} Sortino",
            train_result.total_trades,
            train_result.total_return_pct,
            train_result.sharpe_ratio,
            train_result.sortino_ratio
        );
        println!(
            "Test result: {} trades, {:.2}% return, {:.2} Sharpe, {:.2} Sortino",
            test_result.total_trades,
            test_result.total_return_pct,
            test_result.sharpe_ratio,
            test_result.sortino_ratio
        );
    }
    ...
    fn test_run_single_backtest() {
        ...
        println!(
            "Single backtest: {} trades, {:.2}% return, equity ${:.2}",
