...
mod tests {
    ...
    #[test]
    fn test_run_manifest_new() {
        let config = json!({"symbol": "BTCUSDT", "interval": "1h"});
        let manifest = RunManifest::new("test-run-001".to_string(), config.clone());

        assert_eq!(manifest.run_id, "test-run-001");
        assert_eq!(manifest.status, RunStatus::Running);
        assert!(manifest.completed_at.is_none());
        assert!(manifest.results.is_none());
        assert!(manifest.error.is_none());
        assert_eq!(manifest.config, config);
    }

    #[test]
    fn test_run_manifest_complete() {
        let config = json!({});
        let mut manifest = RunManifest::new("test-run-002".to_string(), config);
        ...
        manifest.complete(results.clone());

        assert_eq!(manifest.status, RunStatus::Completed);
        assert!(manifest.completed_at.is_some());
        assert!(manifest.results.is_some());
        assert!(manifest.duration().is_some());
    }

    #[test]
    fn test_run_manifest_fail() {
        let config = json!({});
        let mut manifest = RunManifest::new("test-run-003".to_string(), config);

        manifest.fail("Something went wrong".to_string());

        assert_eq!(manifest.status, RunStatus::Failed);
        assert!(manifest.completed_at.is_some());
        assert_eq!(manifest.error, Some("Something went wrong".to_string()));
    }

    #[test]
    fn test_run_status_text() {
        assert_eq!(RunStatus::Running.text(), "running");
        assert_eq!(RunStatus::Completed.text(), "completed");
        assert_eq!(RunStatus::Failed.text(), "failed");
        assert_eq!(RunStatus::Cancelled.text(), "cancelled");
    }

    #[test]
    fn test_run_manifest_status_text() {
        let config = json!({});
        let manifest = RunManifest::new("test".to_string(), config);
        assert_eq!(manifest.status_text(), "running");
    }

    #[test]
    fn test_run_manifest_add_output() {
        let config = json!({});
        let mut manifest = RunManifest::new("test".to_string(), config);

        let output = OutputFile {
            file_type: OutputFileType::TradeLog,
            path: PathBuf::from("trades.csv"),
            size_bytes: 1024,
            description: "Trade log".to_string(),
        };

        manifest.add_output(output);
        assert_eq!(manifest.outputs.len(), 1);
        assert_eq!(manifest.outputs[0].file_type, OutputFileType::TradeLog);
    }

    #[test]
    fn test_run_manifest_serialization() {
        let config = json!({"symbol": "ETHUSDT"});
        let manifest = RunManifest::new("serialize-test".to_string(), config);

        let json = serde_json::to_string(&manifest).unwrap();
        let deserialized: RunManifest = serde_json::from_str(&json).unwrap();

        assert_eq!(deserialized.run_id, "serialize-test");
    ...
    fn test_git_info_capture() {
        ...
        // In a git repo, these should not be "unknown"
        assert!(!git_info.commit_hash.is_empty());
        assert!(!git_info.branch.is_empty());
        // commit_hash should be a valid hex string (40 chars for SHA-1)
        assert_eq!(git_info.commit_hash.len(), 40);
    }
    ...
    fn test_environment_info_capture() {
        ...
        assert!(!env_info.hostname.is_empty());
    }

    #[test]
    fn test_run_comparison() {
        let config_a = json!({"stop": 0.05});
        let config_b = json!({"stop": 0.10});

        let mut manifest_a = RunManifest::new("run-a".to_string(), config_a);
        let mut manifest_b = RunManifest::new("run-b".to_string(), config_b);

        let results_a = ResultsSummary {
            best: BacktestMetrics {
                total_return_pct: 50.0,
                sharpe_ratio: 2.0,
                max_drawdown_pct: 10.0,
        ...
        manifest_a.complete(results_a);
        manifest_b.complete(results_b);

        let comparison = manifest_a.compare(&manifest_b);

        assert_eq!(comparison.run_a, "run-a");
        assert_eq!(comparison.run_b, "run-b");
        assert_eq!(comparison.sharpe_diff, Some(0.5)); // 2.0 - 1.5
        assert_eq!(comparison.return_diff, Some(20.0)); // 50.0 - 30.0
        assert_eq!(comparison.drawdown_diff, Some(-5.0)); // 10.0 - 15.0
        assert!(!comparison.config_diff.is_empty()); // stop changed
    }

    #[test]
    fn test_diff_configs_no_change() {
        let a = json!({"symbol": "BTC", "interval": "1h"});
        let b = json!({"symbol": "BTC", "interval": "1h"});
    ...
    #[test]
    fn test_diff_configs_with_change() {
        let a = json!({"symbol": "BTC", "stop": 0.05});
        let b = json!({"symbol": "ETH", "stop": 0.10});

        let diffs = diff_configs(&a, &b);
        assert_eq!(diffs.len(), 2); // Both symbol and stop changed
    }
}
