...
pub fn compute_cs_features(
    ...
    for i in momentum_period..use_len {
        let ret_values: Vec<(&String, f64)> = symbols.iter().map(|s| (*s, returns[s][i])).collect();
        let vol_values: Vec<(&String, f64)> = symbols.iter().map(|s| (*s, vols[s][i])).collect();
...
mod tests {
    ...
    fn test_cs_momentum_rank_ordering() {
        ...
        assert!(
            a_rank > b_rank,
            "A({a_rank:.2}) should rank higher than B({b_rank:.2})"
        );
        assert!(
            b_rank > c_rank,
            "B({b_rank:.2}) should rank higher than C({c_rank:.2})"
        );
    }
