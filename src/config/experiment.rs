...
pub struct CostConfig {
    ...
    /// Optional passive execution configuration
    #[serde(default)]
    pub passive_execution: Option<crate::backtest::passive::PassiveConfig>,
}
...
pub struct SizingConfig {
    ...
    /// Trailing stop percentage (e.g., 0.15 = 15%)
...
mod tests {
    ...
    #[test]
    fn test_experiment_config_example() {
        let config = ExperimentConfig::example();
        assert_eq!(config.name, "btc_trend_following");
        assert!(!config.data.symbols.is_empty());
        assert!(config.validate().is_ok());
    }

    #[test]
    fn test_experiment_config_run_id() {
        let config = ExperimentConfig::example();
        let run_id = config.run_id();

        assert!(run_id.starts_with("btc_trend_following__"));
        assert!(run_id.len() > "btc_trend_following__".len());
    }

    #[test]
    fn test_experiment_config_validate_empty_name() {
    ...
    #[test]
    fn test_experiment_config_validate_empty_symbols() {
        let mut config = ExperimentConfig::example();
        config.data.symbols = vec![];
    ...
    #[test]
    fn test_experiment_config_validate_invalid_ratio() {
        let mut config = ExperimentConfig::example();
        config.validation.train_ratio = 0.7;
        config.validation.test_ratio = 0.5; // 0.7 + 0.5 = 1.2 > 1.0

        let result = config.validate();
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("train_ratio"));
    }

    #[test]
    fn test_experiment_config_validate_excessive_kelly() {
        let mut config = ExperimentConfig::example();
        config.sizing.kelly_fraction = 0.5; // > 0.25

        let result = config.validate();
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Kelly"));
    }

    #[test]
    fn test_cost_config_default() {
        let config = CostConfig::default();
        assert!((config.fee_pct - 0.001).abs() < 1e-10);
        assert!((config.slippage_bps - 5.0).abs() < 1e-10);
        assert!(!config.include_funding);
    }

    #[test]
    fn test_sizing_config_default() {
        let config = SizingConfig::default();
        assert!((config.initial_capital - 10_000.0).abs() < 1e-10);
        assert!((config.position_fraction - 1.0).abs() < 1e-10);
        assert!((config.trailing_stop_pct - 0.15).abs() < 1e-10);
    }

    #[test]
    fn test_validation_config_default() {
        let config = ValidationConfig::default();
        assert_eq!(config.method, "simple_split");
        assert!((config.train_ratio - 0.6).abs() < 1e-10);
        assert!((config.test_ratio - 0.4).abs() < 1e-10);
    ...
    fn test_metrics_config_default() {
        ...
        assert_eq!(config.primary_metric, "sharpe_ratio");
    ...
    fn test_output_config_default() {
        ...
        assert!(config.save_equity_curve);
        assert!(config.save_trades);
    ...
    fn test_experiment_config_serialization() {
        ...
        assert_eq!(deserialized.data.symbols, config.data.symbols);
    }

    #[test]
    fn test_data_config_cache_dir_default() {
        let config = ExperimentConfig::example();
        assert_eq!(config.data.cache_dir, PathBuf::from("./data/cache"));
    }

    #[test]
    fn test_valid_kelly_fraction() {
        let mut config = ExperimentConfig::example();
        config.sizing.kelly_fraction = 0.25; // Exactly at limit
