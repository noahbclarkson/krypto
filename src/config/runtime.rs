...
mod tests {
    ...
    #[test]
    fn test_simple_split_with_purge() {
        let config = ValidationConfig {
            method: "simple_split".to_string(),
            train_ratio: 0.7,
            purge_gap: 20,
        ...
        let split = &splits[0];
        assert_eq!(split.train_range, (0, 700));
        assert_eq!(split.test_range, (720, 1000)); // 700 + 20 purge gap
        assert_eq!(split.purge_range, Some((700, 720)));
    }

    #[test]
    fn test_walk_forward_growing_train() {
        let config = ValidationConfig {
            method: "walk_forward".to_string(),
            n_windows: 4,
            purge_gap: 0,
        ...
        // Each subsequent window should have larger train set
        for i in 1..splits.len() {
            assert!(
                splits[i].train_range.1 > splits[i - 1].train_range.1,
                "Train set should grow in walk-forward"
            );
        }
    }
    ...
    fn test_cpcv_basic() {
        ...
        // Each split should have non-overlapping test ranges
        for i in 0..splits.len() - 1 {
            assert!(
                splits[i].test_range.1 <= splits[i + 1].test_range.0,
    ...
    fn test_cpcv_with_purge() {
        ...
        // Each split should have purge range
        for split in &splits {
            assert!(split.purge_range.is_some());
        }
    }
    ...
    fn test_unknown_method_fails() {
        ...
        let result = compute_splits(&config, 1000);
    ...
    fn test_small_dataset() {
        ...
        let splits = compute_splits(&config, 10).unwrap();
        assert_eq!(splits[0].train_range, (0, 5));
        assert_eq!(splits[0].test_range, (5, 10));
    }

    #[test]
    fn test_data_split_index() {
        let config = ValidationConfig {
            method: "walk_forward".to_string(),
            n_windows: 3,
            purge_gap: 0,
        ...
        let splits = compute_splits(&config, 300).unwrap();

        for (expected_idx, split) in splits.iter().enumerate() {
            assert_eq!(split.index, expected_idx);
        }
    }
}
