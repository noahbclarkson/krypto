...
pub const REGIME_ATR_PERIOD: usize = 64; // hyperopt 2026-05-02: extensive sweep AP∈[5..=80 step 1] × 9 universes × 7 WF windows under live Turtle-only path (T=24). AP=64 wins: 55/63 pass (+2 vs AP=12 baseline), Sharpe 6.19 (+0.76 vs 5.43), DD 19.7% (-7.0pp vs 26.7%). AP=64 is the sole peak — Sharpe drops sharply to AP=65 (3.07) and AP=63 (5.27). AP=12 was found in a prior joint sweep on the dual Chandelier harness; this is a standalone re-optimization on the live Turtle-only path. See memory/hyperopt-2026-05-02-regime-atr-period.md.
