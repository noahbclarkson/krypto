...
impl SignalGenerator for VotingEnsemble {
    ...
    fn predict(&self, df: &DataFrame) -> Result<Series> {
        ...
        let all_signals: Vec<Vec<f64>> = self
            .strategies
            .iter()
        ...
        for i in 0..n {
            let long_votes = all_signals
                .iter()
                .filter(|s| s.get(i).copied().unwrap_or(0.0) > 0.5)
                .count();
            let short_votes = all_signals
                .iter()
                .filter(|s| s.get(i).copied().unwrap_or(0.0) < -0.5)
                .count();
...
impl ConfirmedStrategy {
    ...
    pub fn filter_signal(&self, df: &DataFrame, raw_signal: &Series) -> Result<Series> {
        ...
        for i in self.vol_period..n {
            let avg_vol = volumes[i.saturating_sub(self.vol_period)..i]
                .iter()
                .sum::<f64>()
...
impl SignalGenerator for ConfirmedDynamicTrend {
    ...
    fn explain(&self, df: &DataFrame) -> Result<Series> {
        Ok(Series::new(
            "explanation",
            vec![
                "ConfirmedDynamicTrend: EMA crossover + RSI filter + volume confirmation";
                df.height()
            ],
        ))
    }
...
mod ensemble_tests {
    ...
    #[test]
    fn test_voting_ensemble_n_strategies() {
        use crate::algo::registry::StrategyRegistry;
        let registry = StrategyRegistry::new();
        let strategies: Vec<Box<dyn SignalGenerator>> = vec![
            registry.create("dynamic_trend").unwrap(),
            registry.create("price_momentum").unwrap(),
        ];
        let ensemble = VotingEnsemble::new(strategies, 2);
        assert_eq!(ensemble.n_strategies(), 2);
        assert_eq!(ensemble.min_agree, 2);
    }

    #[test]
    fn test_confirmed_strategy_no_volume_filter() {
        let n = 30;
        let raw = Series::new("signal", vec![1.0f64; n]);
        let df = DataFrame::new(vec![
            Series::new("close", vec![100.0f64; n]),
            Series::new("volume", vec![100.0f64; n]), // very low volumes
        ])
        .unwrap();

        // With use_vol_confirm = false, signal should pass through unchanged
        let filter = ConfirmedStrategy {
            vol_multiplier: 1.5,
            vol_period: 20,
        ...
        assert!(
            ...
            "With vol_confirm disabled, all signals should pass"
        );
    }
    ...
    fn test_confirmed_dynamic_trend_compiles() {
        ...
        assert!(
            result.is_ok(),
            "ConfirmedDynamicTrend::predict should not error"
        );
    }

    #[test]
    fn test_meta_ensemble_no_strategies() {
        let ensemble = MetaEnsemble::new();
        // Without strategies, signal should be 0.0
        let df = make_df(10);
        let result = ensemble.generate_signal(&df).unwrap();
        assert_eq!(
            result, 0.0,
            "MetaEnsemble with no strategies should return 0.0"
        );
    }

    #[test]
    fn test_voting_ensemble_name() {
        let ensemble = VotingEnsemble::new(vec![], 1);
        assert_eq!(ensemble.name(), "VotingEnsemble");
    }
}
