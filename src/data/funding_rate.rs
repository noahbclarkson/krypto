...
impl FundingRateLoader {
    ...
    async fn fetch_paginated(
        ...
        let end_ms = end_time.map(|dt| dt.timestamp_millis()).unwrap_or(i64::MAX);
...
pub fn align_to_ohlcv(
    ...
    let fund_times = funding_with_features
        .column("time")?
        .cast(&DataType::Int64)?;
    let fund_ts: Vec<i64> = fund_times
        .i64()?
        .into_iter()
        .map(|v| v.unwrap_or(0))
        .collect();

    let fund_rate: Vec<f64> = funding_with_features
        .column("funding_rate")?
        .f64()?
        .into_iter()
        .map(|v| v.unwrap_or(0.0))
        .collect();
    let fund_z: Vec<f64> = funding_with_features
        .column("funding_rate_z")?
        .f64()?
        .into_iter()
        .map(|v| v.unwrap_or(0.0))
        .collect();
    let fund_ma: Vec<f64> = funding_with_features
        .column("funding_rate_ma")?
        .f64()?
        .into_iter()
        .map(|v| v.unwrap_or(0.0))
        .collect();
    let fund_std: Vec<f64> = funding_with_features
        .column("funding_rate_std")?
        .f64()?
        .into_iter()
        .map(|v| v.unwrap_or(0.0))
        .collect();
    let fund_extreme: Vec<f64> = funding_with_features
        .column("funding_extreme")?
        .f64()?
        .into_iter()
        .map(|v| v.unwrap_or(0.0))
        .collect();
...
mod tests {
    ...
    fn test_funding_features_extreme_positive() {
        ...
        let mut rates: Vec<f64> = (0..30)
            .map(|i| 0.0001 + (i as f64 % 5.0) * 0.00005)
            .collect();
        ...
        assert_eq!(
            extremes.get(29).unwrap(),
            1.0,
            "Expected extreme positive at last bar"
        );
    }
    ...
    fn test_funding_features_extreme_negative() {
        ...
        assert_eq!(
            extremes.get(29).unwrap(),
            -1.0,
            "Expected extreme negative at last bar"
        );
    }
