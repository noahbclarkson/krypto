...
impl DataLoader {
    ...
    pub async fn fetch_data(
        ...
        while remaining > 0 {
            ...
            if batch.is_empty() {
                tracing::warn!(
                    "No more data available from Binance (request {})",
                    requests_made
                );
                break;
            }
    ...
    pub async fn fetch_with_cache(
        ...
        if let Some(cached) = self.load_from_cache(symbol, interval)? {
            if cached.height() >= total_candles as usize {
                ...
                return Ok(cached.slice(
                    (cached.height() - total_candles as usize) as i64,
    ...
    fn klines_to_dataframe(
        &self,
        klines: Vec<binance::rest_model::KlineSummary>,
    ) -> Result<DataFrame> {
        let n = klines.len();
    ...
    fn validate_dataframe(&self, df: &DataFrame, symbol: &str) -> Result<()> {
        ...
        for col in &required {
            df.column(col).with_context(|| {
                format!("Missing required column '{}' in data for {}", col, symbol)
            })?;
        }

        // Check for reasonable price values (not zero/negative)
        let close = df.column("close")?;
        let close_f64 = close.f64().context("Close column should be f64")?;
        ...
        let time_dt = time.datetime().context("Time column should be datetime")?;
...
mod tests {
    ...
    #[test]
    fn test_cache_config_custom_dir() {
    ...
    #[test]
    fn test_dataloader_with_custom_cache() {
        let custom_cache = CacheConfig {
            cache_dir: "test/cache".to_string(),
            enabled: false,
        };
        let loader = DataLoader::with_cache(None, None, custom_cache.clone());
        assert!(!loader.cache.enabled);
        assert_eq!(loader.cache.cache_dir, "test/cache");
    }

    #[test]
    fn test_cache_key_lowercase() {
        let loader = DataLoader::new(None, None);
        let key = loader.cache_key("ETHUSDT", "4h");
        assert!(key.contains("ethusdt"));
        assert!(!key.contains("ETHUSDT"));
    }

    #[test]
    fn test_cache_key_path_format() {
        let loader = DataLoader::new(None, None);
        let key = loader.cache_key("SOLUSDT", "1d");
        // Should be: cache_dir/symbol_interval.parquet
        assert!(key.starts_with("data/cache/"));
        assert!(key.ends_with("solusdt_1d.parquet"));
    }

    #[test]
    fn test_load_from_cache_disabled() {
        let cache = CacheConfig {
            enabled: false,
            ..Default::default()
        };
        let loader = DataLoader::with_cache(None, None, cache);
        let result = loader.load_from_cache("BTCUSDT", "1h").unwrap();
    ...
    fn test_load_from_cache_missing_file() {
        let cache = CacheConfig {
            enabled: true,
            cache_dir: "/nonexistent/path".to_string(),
        };
        let loader = DataLoader::with_cache(None, None, cache);
        let result = loader.load_from_cache("BTCUSDT", "1h").unwrap();
        assert!(result.is_none());
