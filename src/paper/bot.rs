...
impl PaperBot {
    ...
    fn close_position(&mut self, exit_price: f64, exit_time: DateTime<Utc>) -> CompletedTrade {
        let (entry_price, size, is_long, entry_time) = match &self.position {
            Position::Flat => panic!("Cannot close flat position"),
            Position::Long { entry_price, size, .. } => {
                (*entry_price, *size, true, self.history.back().map(|b| b.time).unwrap_or_default())
            }
            Position::Short { entry_price, size, .. } => {
                (*entry_price, *size, false, self.history.back().map(|b| b.time).unwrap_or_default())
            }
        };
    ...
    pub fn summary(&self) -> BotSummary {
        ...
        let directionally_correct = self.trades.iter()
            .filter(|t| t.pnl > 0.0)
            .count();
...
mod tests {
    ...
    fn make_bar(close: f64, open: f64) -> Bar {
        Bar::new(Utc::now(), open, close.max(open), close.min(open), close, 1000.0)
    }
