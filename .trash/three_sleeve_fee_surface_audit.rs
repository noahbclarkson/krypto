...
const LOAD_SYMBOLS: &[&str] = &[
    "BTCUSDT", "ETHUSDT", "SOLUSDT", "XRPUSDT", "DOGEUSDT", "ADAUSDT", "LTCUSDT",
    "BNBUSDT", "EOSUSDT", "BCHUSDT",
];
const UNIVERSES: &[(&str, &[&str])] = &[
    ("Base5", &["ETHUSDT", "SOLUSDT", "XRPUSDT", "DOGEUSDT", "ADAUSDT"]),
    ...
    ("LargeCaps5", &["ETHUSDT", "SOLUSDT", "XRPUSDT", "BNBUSDT", "ADAUSDT"]),
...
async fn main() -> Result<()> {
    ...
    println!("Execution: signal at close, entry next open, exit after {} bars at open", HOLD_BARS);
    println!("Fees: {:.1}% taker each side", TAKER_FEE * 100.0);
    println!("Portfolio lens: top-{} capped book inside each sleeve, then sleeve-level DDHard budgeting", POSITION_CAP);
    println!("Honesty note: SmallByDollarVol = inverse dollar-volume proxy, not true market cap.\n");
    ...
    for &(label, universe_symbols) in UNIVERSES {
        println!("\n--- Universe: {} ({}) ---", label, universe_symbols.join(", "));
        ...
        rows.sort_by(|a, b| {
            b.resample_pass
                ...
                .then_with(|| b.full.aligned_return_pct.partial_cmp(&a.full.aligned_return_pct).unwrap())
        });

        println!("{:<24} {:>8} {:>8} {:>10} {:>10} {:>8} {:>8}", "Portfolio", "QPass", "RPass", "FullRet%", "FullDD%", "Sharpe", "Trades");
        println!("{}", "-".repeat(92));
        for row in &rows {
            println!(
        ...
        universe_summaries.push(UniverseSummary { label: label.to_string(), rows });
    }
...
fn aligned_universe(raw_cache: &HashMap<String, DataFrame>, symbols: &[&str]) -> Result<UniverseData> {
    let min_len = symbols
        .iter()
...
fn generate_small_by_dollar_volume_signal_strengths(
    ...
    let n = universe.data.iter().map(|(_, df)| df.height()).min().unwrap_or(0);
    ...
    let symbols = universe.data.iter().map(|(s, _)| s.clone()).collect::<Vec<_>>();
    ...
    for i in 0..n {
        let values = symbols.iter().map(|s| raw_scores.get(s).unwrap()[i]).collect::<Vec<_>>();
        ...
        for (idx, symbol) in symbols.iter().enumerate() {
            ...
            let signal = if z > 0.35 { 1 } else if z < -0.35 { -1 } else { 0 };
            let strength = z.abs();
...
fn build_symbol_plan(
    ...
    while i + HOLD_BARS + 1 < n {
        ...
        let gross_return = if signal > 0 { exit / entry - 1.0 } else { entry / exit - 1.0 };
...
fn simulate_family_portfolio_window(
    ...
    for local_day in 0..steps {
        ...
        for sleeve in sleeves {
            ...
            let dd = if pk > 0.0 { (1.0 - eq / pk) * 100.0 } else { 0.0 };
            let rec = if pk > 0.0 { eq / pk } else { 1.0 };
            let weight = sleeve_weight(portfolio, sleeve.strategy, dd, rec, global_dd, global_recovery);
    ...
    let avg_active_positions = active_counts.iter().sum::<usize>() as f64 / active_counts.len() as f64;
    let idle_days_pct = active_counts.iter().filter(|&&c| c == 0).count() as f64 / active_counts.len() as f64 * 100.0;
    let win_rate_pct = if trade_count == 0 { 0.0 } else { wins as f64 / trade_count as f64 * 100.0 };
...
fn included_strategies(portfolio: PortfolioKind) -> &'static [StrategyKind] {
    match portfolio {
        ...
        PortfolioKind::DdBudgetAllThree => {
            &[StrategyKind::AdMomentum, StrategyKind::MacdRegime, StrategyKind::SmallByDollarVol]
        }
    }
}
...
fn sleeve_weight(
    ...
    match portfolio {
        PortfolioKind::AdOnly if strategy == StrategyKind::AdMomentum => ddhard_exposure(dd_pct, recovery_ratio),
        PortfolioKind::MacdOnly if strategy == StrategyKind::MacdRegime => ddhard_exposure(dd_pct, recovery_ratio),
        PortfolioKind::SmallOnly if strategy == StrategyKind::SmallByDollarVol => ddhard_exposure(dd_pct, recovery_ratio),
        PortfolioKind::EqualAdMacd if matches!(strategy, StrategyKind::AdMomentum | StrategyKind::MacdRegime) => 0.5,
        PortfolioKind::EqualAdSmall if matches!(strategy, StrategyKind::AdMomentum | StrategyKind::SmallByDollarVol) => 0.5,
        PortfolioKind::EqualMacdSmall if matches!(strategy, StrategyKind::MacdRegime | StrategyKind::SmallByDollarVol) => 0.5,
        PortfolioKind::DdBudgetAdMacd => match strategy {
            StrategyKind::AdMomentum | StrategyKind::MacdRegime => 0.50 * ddhard_exposure(dd_pct, recovery_ratio),
            _ => 0.0,
        },
        PortfolioKind::DdBudgetAdSmall => match strategy {
            StrategyKind::AdMomentum | StrategyKind::SmallByDollarVol => 0.50 * ddhard_exposure(dd_pct, recovery_ratio),
            _ => 0.0,
        },
        PortfolioKind::DdBudgetMacdSmall => match strategy {
            StrategyKind::MacdRegime | StrategyKind::SmallByDollarVol => 0.50 * ddhard_exposure(dd_pct, recovery_ratio),
            _ => 0.0,
        },
...
fn generate_macd_regime_signals(df: &DataFrame) -> Result<Vec<i32>> {
    ...
    for i in 0..df.height() {
        ...
        if sma_now <= 0.0 { continue; }
        if macd_now > macd_sig_now && price > sma_now { out[i] = 1; }
        else if macd_now < macd_sig_now && price < sma_now { out[i] = -1; }
    }
    Ok(out)
}
...
fn rolling_dollar_volume(close: &Float64Chunked, volume: &Float64Chunked, lookback: usize) -> Vec<f64> {
...
fn calculate_sma(values: &Float64Chunked, period: usize) -> Vec<f64> {
    ...
    for i in 0..values.len() {
        ...
        if i >= period { sum -= values.get(i - period).unwrap_or(0.0); }
        if i + 1 >= period { out[i] = sum / period as f64; }
    }
    out
}
...
fn quarter_windows(steps: usize) -> Vec<(usize, usize)> {
    ...
    for i in 0..4 {
        ...
        let end = if i == 3 { steps } else { usable_start + (i + 1) * chunk };
        if end > start {
            out.push((start, end));
        }
    }
...
fn cpcv_style_windows(steps: usize) -> Vec<(usize, usize)> {
    ...
    for picked in choose_indices(BLOCKS, TEST_BLOCKS) {
        ...
        let end = if end_block >= BLOCKS { steps } else { usable_start + end_block * block };
        if end > start {
...
fn write_snapshot(universe_summaries: &[UniverseSummary]) -> Result<(String, String)> {
    ...
    let archive_md = format!("{}/factor_small_sleeve_chronology_audit_{}.md", SNAPSHOT_DIR, ts);
    let archive_csv = format!("{}/factor_small_sleeve_chronology_audit_{}.csv", SNAPSHOT_DIR, ts);
