...
**State: 2026-05-06 03:25 UTC — T53 MOCK EXCHANGE AUDIT / EXECUTION BLOCKER CONFIRMED**

## Current Truth

- **Exact as-coded live bot (T65/T67, regenerated 2026-05-06 03:19):** 2.55x / daily account Sharpe 0.95 / MaxDD 28.8% / 298 trades / 1,795 Base5 days.
- **Semantic gap UNDERDIAGNOSED — 70x equity difference unexplained:** Research harness = 176.79x / Sharpe 3.29 / MaxDD 99.5% / 156 trades. Live bot (exact) = 2.55x / Sharpe 0.95 / MaxDD 28.8% / 298 trades. T69 semantic patch (strict prior-window Turtle + VL ranking + size-aware accounting) made it WORSE → 1.02x. **We don't know which research-harness-validated parameters (EP=21, AP=17, LB=41, VL=92) actually apply to the live bot path.**
- **Top-trade concentration risk:** Top 10 trades = 91.3% of log return. Top 5 = 57.7%. A filter that accidentally excludes 2-3 winners reduces 2.55x to ~1.5x.
...
**Status:** AUDITED, STILL UNBUILT. 5+ weeks overdue.
- `src/live/mock_exchange.rs` is substantial (orders/fills/positions/fee/slippage config), but it is not wired into `src/live/bot.rs`.
- `mock_live_bot.rs` does not compile (`symbol_str` undefined + type mismatch) and contains debug/stale strategy code.
- `mock_live_bot_v2.rs` compiles/runs, but it is **not** an end-to-end wire replacement for Binance WebSocket and not valid execution-cost evidence: TAKER/MAKER/REALISTIC configs produce identical economics because cash/equity accounting ignores actual `MockExchange` fills/fees/slippage.
- Original design gap: no cached 1m parquet exists (cache has daily/4h only), so a local HTTP/WS server seeded from historical 1m needs either a 1m downloader/cache first or a daily-bar mock-feed scope reduction.
- Goal remains: `src/live/bot.rs` → mock feed/executor → verify same signals as T65 harness on the same data. This unblocks real execution feedback: fills, slippage, order state, disconnect/reconnect.
...
- Clean headline: **exact live bot = 2.55x / daily account Sharpe 0.95 / MaxDD 28.8% / 298 trades / 1,795 days**.
...
- Verdict: do **not** patch `src/live/bot.rs` with this candidate. Exact as-coded live bot remains stronger at 2.55x.
